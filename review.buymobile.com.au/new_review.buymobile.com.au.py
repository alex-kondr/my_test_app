...
def run(context: dict[str, str], session: Session):
    ...
    session.queue(Request('https://buymobile.com.au/', use='curl'), process_frontpage, dict())
...
def process_product(data: Response, context: dict[str, str], session: Session):
    ...
    if product.ssid:
        ...
        session.do(Request(revs_url, max_age=0, use='curl'), process_reviews, dict(product=product))
...
def process_reviews(data: Response, context: dict[str, str], session: Session):
    ...
    if next_url:
        ...
        session.do(Request(revs_url, max_age=0, use='curl'), process_reviews, dict(product=product, page=next_page))
