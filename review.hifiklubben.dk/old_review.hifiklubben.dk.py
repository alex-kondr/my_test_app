...
def process_frontpage(data, context, session):
    ...
    cats = data.xpath('//ul[contains(@class, "iOLOpE")]/li/a')
    for cat in cats:
        name = cat.xpath('span[contains(@class, "dNlIYv")]/text()').string()
...
def process_category(data, context, session):
    ...
    subcats = data.xpath('//nav/ul[contains(@class, "dGDWjo")]/li/a')
    for subcat in subcats:
        name = subcat.xpath('div[contains(@class, "bPKKSR")]/text()').string()
        url = subcat.xpath('@href').string()
        session.queue(Request(url), process_prodlist, dict(cat=context['cat']+'|'+name, url=url))


def process_prodlist(data, context, session):
    strip_namespace(data)

    prods_cnt = data.xpath('//div[contains(@class, "bUTrda")]/div/text()').string()
    if not prods_cnt:
        process_product(data, context, session)

    prods = data.xpath('//div/article[contains(@class, "joPMea")]')
    for prod in prods:
        ...
        if revs_cnt and int(revs_cnt) > 0:
            session.queue(Request(url), process_prodlist, dict(context, name=name, url=url))
