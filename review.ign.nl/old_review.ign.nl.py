...
def process_review(data: Response, context: dict[str, str], session: Session):
    ...
    if not name:
        name = context['title']
    if name == 'All':
        name = context['title'].split('DVD Review:')[-1].split('DVD-Review:')[-1].split(' - Filmreview')[0].split('Blu-ray Review:')[-1].strip()
    elif name == 'Pc':
    ...
    if next_url:
        page = 1
        title = review.title + " - Bladzijde " + str(page)
...
def process_review_next(data: Response, context: dict[str, str], session: Session):
    ...
    if not grade_overall:
        grade_overall = data.xpath('//p[contains(., "Cijfer:") or contains(., "Score:")]//text()').string(multiple=True)
    if not grade_overall:
        grade_overall = data.xpath('//div[strong[contains(., "Cijfer:") or contains(., "Score:")]]//text()').string(multiple=True)
    if grade_overall:
        grade_overall = grade_overall.split('Cijfer:')[-1].split('Score:')[-1].split('van')[0].split('uit')[0].strip().split()[0].replace(',', '.')
        if float(grade_overall) > 10:
            grade_overall = grade_overall[0] + '.' + grade_overall[1]
    ...
    if page > 1:
        ...
        excerpt = data.xpath('//div[@id="id_text"]/p[not(preceding::div[contains(text(), "Het oordeel") or contains(text(), "Conclusie")])]//text()[not(contains(., "Conclusie") or contains(., "beoordeling voor"))]').string(multiple=True)
        if not excerpt:
            excerpt = data.xpath('//div[@id="id_text"]//text()[not(ancestor::div[contains(text(), "Het oordeel") or contains(text(), "Conclusie")] or preceding::div[contains(text(), "Het oordeel") or contains(text(), "Conclusie")])][not(contains(., "Conclusie"))]').string(multiple=True)
        if excerpt:
            if conclusion:
                excerpt = excerpt.replace(conclusion.strip(), '')
    ...
    if next_url:
        ...
        if context.get('conclusion'):
            conclusion = serialize_text(context['conclusion'].encode("ascii", errors="ignore")).strip()
            review.add_property(type='conclusion', value=conclusion)

        if context.get('excerpt'):
            excerpt = serialize_text(context['excerpt']).strip()
