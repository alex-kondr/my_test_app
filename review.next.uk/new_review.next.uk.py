...
def run(context, session):
    ...
    session.queue(Request('https://www.next.co.uk/shop/department-homeware-productaffiliation-kitchen-0', max_age=0), process_category, dict(cat='Home|Kitchen & Dining'))
    session.queue(Request('https://www.next.co.uk/shop/department-homeware-productaffiliation-lighting-0', max_age=0), process_category, dict(cat='Home|Lighting'))
...
def process_reviews(data, context, session):
    ...
    for rev in revs:
        if rev.get('Review', {}).get('Imported', False):
            continue

        review = Review()
        # review.title = rev['Review'].get('Title') — prod.name
        review.type = 'user'
        review.ssid = rev.get('Review', {}).get('SubmissionId')
        review.url = product.url

        date = rev.get('Review', {}).get('SubmissionTime')
        if date:
        ...
        is_recommended = rev.get('Review', {}).get('IsRecommended')
        if is_recommended:
            review.add_property(value=True, type='is_recommended')

        excerpt = rev.get('Review', {}).get('Text')
        if excerpt:
    ...
    if next_page and page < int(next_page):
        ...
        session.do(Request(revs_url, max_age=0), process_reviews, dict(context, product=product, page=next_page))
