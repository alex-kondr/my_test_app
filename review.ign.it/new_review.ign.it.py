...
def process_review(data: Response, context: dict[str, str], session: Session):
    ...
    product.name = data.xpath('//div[@class="object-breadcrumbs"]/a[not(regexp:test(., "Cinema/TV|Amazon|Online|Mobile"))]/text()').string()
    if not product.name:
        product.name = context['title'].strip()

    platforms = data.xpath('//li[@class="platform"]/@data-platform').join('/')
    if platforms:
        product.category += '|' + platforms.replace('-', ' ').upper().strip()
