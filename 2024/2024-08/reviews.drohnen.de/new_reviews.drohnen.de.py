...
def process_review(data, context, session):
    product = Product()
    product.name = context['title'].replace('[Test & Vergleich]', '').replace(': Test / Vergleich / Bewertung', '').replace('– Test / Vergleich / Bewertung', '').replace('– Test / Vergleich', '').replace(': Test / Vergleich und Angebote', '').replace('Vergleich und Test', '').replace(': Test, Vergleich und Erfahrungen', '').replace('im Test & Vergleich', '').replace(': Test & Erfahrungen', '').replace(': Test & Erfahrungen', '').replace(': Test der Enterprise-Drohne', '').replace(': Test & Review', '').replace(': Test und Angebote', '').replace('| Tests und Ratgeber', '').replace(u'\u2013 Preise, Test und Infos', '').replace('| Empfehlungen, Testberichte 2024', '').replace(': Test und Features', '').replace(': Test, Erfahrung und Preis', '').replace(u'\u2013 Test und Angebote', '').replace(': Test und Erfahrungen', '').replace(': Test und Erfahrung', '').replace('im Praxistest', '').replace('im Test', '').strip()
