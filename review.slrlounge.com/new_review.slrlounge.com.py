...
def process_review(data, context, session):
    ...
    product.category = data.xpath('//a[contains(@class, "gb-button-5d91b971") and not(regexp:test(., "Reviews|News|Best"))]/text()').join('|')
    ...
    conclusion = data.xpath('((//h2|//h3)[regexp:test(., "Conclusion|Verdict|final", "i")])[last()]/following-sibling::p[not(.//strong[text()="B&H" or text()="Adorama" or text()="Amazon"] or regexp:test(., "Article written by:|Pre-Order the"))]//text()').string(multiple=True)
    if conclusion:
        conclusion = conclusion.replace('â€¢', '').strip()
        review.add_property(type='conclusion', value=conclusion)

    excerpt = data.xpath('(//h2|//h3)[regexp:test(., "Conclusion|Verdict|final", "i")]/preceding-sibling::p[not(.//strong[text()="B&H" or text()="Adorama" or text()="Amazon"] or regexp:test(., "Article written by:|Pre-Order the"))]//text()').string(multiple=True)
    if not excerpt:
