...
def run(context, session):
    session.queue(Request('https://pixel.tv/category/gaming/', force_charset='utf-8', max_age=0), process_revlist, dict())
...
def process_revlist(data, context, session):
    ...
    for rev in revs:
        ...
        session.queue(Request(url, force_charset='utf-8', max_age=0), process_review, dict(title=title, author=author, date=date, url=url))
