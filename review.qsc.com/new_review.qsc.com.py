    ...
    for cat in cats:
        ...
        if name not in XCAT:
            ...
            for sub_cat in sub_cats:
                sub_name = sub_cat.xpath('button/text()').string()

                sub_cats1 = sub_cat.xpath('.//div[@class="csc-typography"]')
                for sub_cat1 in sub_cats1:
                    sub_name1 = sub_cat1.xpath('.//p[@class="bodytext"]//text()').string()

                    sub_cats2 = sub_cat1.xpath('.//ul//a')
                    for sub_cat2 in sub_cats2:
                        prod_name = sub_cat2.xpath('text()').string()
                        url = sub_cat2.xpath('@href').string()
                        cat = name + '|' + sub_name + ('|' + sub_name1 if sub_name1 and sub_name1 != sub_name else '')
