...
def process_prodlist(data, context, session):
    ...
    for prod in prods:
        product = Product()
        product.name = prod.xpath('.//div/span[@class="brandSubDescription"]/text()').string()
        product.manufacturer = prod.xpath('.//div/span[@class="brand heavy"]/text()').string()
        product.url = prod.xpath('.//a[@class="productDescLink"]/@href').string()
        product.category = context['cat']
        product.ssid = prod.xpath('div/@id').string() or prod.xpath('div/@data-product-id').string()
        product.sku = product.ssid

        revs_cnt = prod.xpath('.//div[contains(@class, "productReviewsCount")]/text()').string()
        if revs_cnt:
