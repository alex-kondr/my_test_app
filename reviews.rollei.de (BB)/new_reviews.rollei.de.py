...
def run(context: dict[str, str], session: Session):
    ...
    session.queue(Request('https://www.rollei.de/collections/', use='curl', force_charset='utf-8', max_age=0), process_catlist, dict())
...
def process_catlist(data: Response, context: dict[str, str], session: Session):
    ...
    if next_url:
        session.queue(Request(next_url, use='curl', force_charset='utf-8', max_age=0), process_catlist, dict())
    ...
    for prod in prods:
        ...
        if revs_cnt:
            ...
            if revs_cnt.isdigit() and int(revs_cnt) > 0:
                ...
                session.queue(Request(url, use='curl', force_charset='utf-8', max_age=0), process_product, dict(context, name=name, url=url, revs_cnt=int(revs_cnt)))
