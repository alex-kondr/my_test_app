...
def run(context: dict[str, str], session: Session):
    ...
    session.queue(Request('https://www.hifitest.de/testberichte', force_charset='utf-8', use='curl'), process_revlist, dict())
...
def process_revlist(data: Response, context: dict[str, str], session: Session):
    ...
    for rev in revs:
        ...
        session.queue(Request(url, force_charset='utf-8', use='curl'), process_review, dict(url=url, title=title))
