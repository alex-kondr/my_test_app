...
def process_frontpage(data, context, session):
    ...
    for cat in cats:
        ...
        if name not in XCAT:
            cats1 = cat.xpath('.//div[@class="single-level-menu-list__entry" and div/span[contains(text(), "Kategorie")]]/a')
            if cats1:
                for cat1 in cats1:
                ...
                cats1 = cat.xpath('.//div[@class="single-level-menu-list__entry"]')
                for cat1 in cats1:
                    cat1_name = cat1.xpath('div//text()').string(multiple=True)

                    subcats = cat1.xpath('a')
