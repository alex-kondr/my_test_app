...
def run(context: dict[str, str], session: Session):
    ...
    session.queue(Request('https://www.dday.it/prove', force_charset='utf-8', max_age=0), process_revlist, dict())
...
def process_revlist(data: Response, context: dict[str, str], session: Session):
    ...
    for rev in revs:
        ...
        session.queue(Request(url, force_charset='utf-8', max_age=0), process_review, dict(title=title, cat=cat, url=url))
...
def process_review(data: Response, context: dict[str, str], session: Session):
    ...
    if rev_info:
        ...
        session.do(Request(next_url, force_charset='utf-8', max_age=0), process_review_next, dict(product=product, review=review, excerpt=excerpt))
