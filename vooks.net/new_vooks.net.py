...
def process_review(data, context, session):
    ...
    author = data.xpath('(//span[@class="author"]/a|//a[@rel="author"])//text()').string()
    author_url = data.xpath('(//span[@class="author"]/a|//a[@rel="author"])/@href').string()
    ...
    pros = data.xpath('//div[@class="procon pro"]/p')
    if not pros:
        pros = data.xpath('//div[h4[contains(., "The Good")]]/div')
    ...
    cons = data.xpath('//div[@class="procon con"]/p')
    if not cons:
        cons = data.xpath('//div[h4[contains(., "The Bad")]]/div')
    ...
    if not conclusion:
        conclusion = data.xpath('//div[h4[contains(., "Our Verdict")]]//p//text()').string(multiple=True)
    ...
    if not excerpt:
        excerpt = data.xpath('//div[@class="post-content"]/p[not(contains(., "Rating:"))]//text()').string(multiple=True)
