...
def process_revlist(data, context, session):
    ...
    for rev in revs:
        ...
        session.queue(Request(url + '?seite=all', use='curl', force_charset='utf-8', max_age=0), process_review, dict(url=url))
    ...
    elif not context.get('repeate'):
        time.sleep(SLEEP)
        session.do(Request(data.response_url, use='curl', force_charset='utf-8', max_age=0), process_revlist, dict(context, repeate=True))
...
def process_review(data, context, session):
    ...
    conclusion = data.xpath('(//h3[contains(., "Fazit")])[last()]/following-sibling::p[preceding-sibling::h3[1][contains(., "Fazit")]]//text()').string(multiple=True)
    if conclusion:
        review.add_property(type='conclusion', value=conclusion)

    excerpt = data.xpath('//h3[contains(., "Fazit")]/preceding-sibling::p//text()').string(multiple=True)
    if not excerpt:
        excerpt = data.xpath('//div[@class="article-content"]/p[not(preceding::h3[contains(., "Technische Informationen")])]//text()').string(multiple=True)
