...
def run(context: dict[str, str], session: Session):
    ...
    session.queue(Request('https://www.psillustrated.com/psillustrated/index_reviews.php/1', force_charset='utf-8', max_age=0, use='curl'), process_revlist, dict())
...
def process_revlist(data: Response, context: dict[str, str], session: Session):
    ...
    for rev in revs:
        ...
        session.queue(Request(url, use='curl', force_charset='utf-8', max_age=0), process_review, dict(title=title, platform=platform, url=url))

    next_url = data.xpath('//a[contains(text(), "Next")]/@href').string()
    if next_url:
        session.queue(Request(next_url, force_charset='utf-8', max_age=0, use='curl'), process_revlist, dict())
