...
def run(context, session):
    ...
    session.queue(Request('https://dailynintendo.nl/category/reviews/'), process_revlist, dict())
...
def process_review(data, context, session):
    ...
    product = Product()
    product.name = re.sub(r'\([^\(\)]+\)', '', context['title']).replace('[Preview] – ', '').split(' – ')[0].replace('[Review]', '').replace('[REVIEW]', '').replace('[REVIEW ]', '').replace('[Preview]', '').replace('[PREVIEW]', '').replace('[SWITCH REVIEW]', '').replace('[N3DS REVIEW]', '').replace('[Gamescom] Preview:', '').replace('[Gamescom] Preview', '').replace('[Gamescom] ', '').replace(' Preview', '').replace('3DS Review: ', '').replace('3DS eShop Review:', '').replace('Wii Review: ', '').replace('3DS Video Review: ', '').replace('DS Review: ', '').replace('Wii/3DS review: ', '').replace('Wii U eShop Review: ', '').replace('Wii U eshop review: ', '').replace('Wii U Review: ', '').replace('New 3DS review: ', '').replace('Review: ', '').replace('[Mini-Review]', '').replace('Wii U review:', '').replace('“Review”:', '').replace(' review online!', '').replace('[3DS XL review]', '').replace('[Mini Review] ', '').replace('3ds eshop review: ', '').replace('3DS eShop review: ', '').strip()
    ...
    pros = data.xpath('//p[contains(@class, "has-vivid-green") or .//*[@style="color: #339966;"]]//text()[normalize-space(.)]').string(multiple=True)
    if pros:
        pros = pros.split('+ ')
        for pro in pros:
            pro = pro.strip(' +-*.:;•,–')
    ...
    cons = data.xpath('//p[@style="color:#f93f3f" or contains(@class, "has-vivid-red-color") or .//*[@style="color: #ff0000;"]]//text()[normalize-space(.)]').string(multiple=True)
    if cons:
        cons = cons.split('– ')
        for con in cons:
            con = con.strip(' +-*.:;•,–')
    ...
    conclusion = data.xpath('((//h2|//p|//h4)[regexp:test(., "conclusie", "i")])[last()]/following-sibling::p[not(regexp:test(., "Graphics|Gameplay|Eindcijfer|Beoordeling|Geluid|Speelduur|Totaal|Cijfer:|@") or .//span[@style="color: #339966;" or @style="color: #ff0000;" or @style="color: #ff9900;"])]//text()').string(multiple=True)
    if not conclusion:
    ...
    excerpt = data.xpath('(//h2|//p|//h4)[regexp:test(., "conclusie", "i")]/preceding-sibling::p//text()').string(multiple=True)
    if not excerpt:
        excerpt = data.xpath('//div[contains(@class, "post-content")]/p[not(regexp:test(., "Graphics|Gameplay|Eindcijfer|Beoordeling|Geluid|Speelduur|Totaal|Cijfer:|@") or .//span[@style="color: #339966;" or @style="color: #ff0000;" or @style="color: #ff9900;"])]//text()').string(multiple=True)
