...
XCAT = ['Home', 'New In & Offers', 'Gifts', 'Shop All', 'Shop all']


def run(context, session):
    session.sessionbreakers = [SessionBreak(max_requests=5000)]
    session.queue(Request('https://thedropstore.com/'), process_frontpage, dict())
...
def process_frontpage(data, context, session):
    ...
    for cat in cats:
        ...
        if name and name not in XCAT:
            sub_cats = cat.xpath('ul[@class="nav-dropdown"]/li[@class="nav-dropdown__item"]/a')
...
def process_reviews(data, context, session):
    ...
    response = data.xpath("//body/text()").string()
