...
def run(context: dict[str, str], session: Session):
    ...
    session.queue(Request('https://services.mybcapps.com/bc-sf-filter/filter?t=1788788566887&_=pf&shop=uk-avon.myshopify.com&page=2&limit=24&sort=manual&locale=en&event_type=collection&build_filter_tree=true&sid=c89b4436-651a-44ca-a894-6f6b1c93de69&pg=collection_page&zero_options=true&product_available=false&variant_available=false&sort_first=available&urlScheme=2&collection_scope=2073', use="curl"), process_prodlist, dict(category='Womens Fragrance'))
    session.queue(Request('https://avon.uk.com/collections/body-sprays-mists', use="curl"), process_prodlist, dict(category='Body Sprays'))
    session.queue(Request('https://avon.uk.com/collections/fragrance/mens_aftershave', use="curl"), process_prodlist, dict(category='Mens Fragrance'))
