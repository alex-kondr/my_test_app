...
def run(context, session):
    ...
    session.queue(Request("https://www.luggage.co.nz/"), process_frontpage, dict())


def process_frontpage(data, context, session):
    cats = data.xpath('//li[contains(@class, "level0")]/a[@class="level-top parent"]')
    for cat in cats:
...
def process_catlist(data, context, session):
    subcats = data.xpath('//li[contains(@class, "amshopby-cat amshopby-cat-level-2")]/a')
    for subcat in subcats:
        name = context['cat'] + "|" + subcat.xpath('text()').string(multiple=True)
