...
def process_frontpage(data, context, session):
    ...
    for cat in cats:
        name = cat.xpath('div//a[contains(., "Alle ")]//text()').string(multiple=True)

        if name:
            name = name.replace('Alle ', '').strip()

            subcats = cat.xpath('div//a[not(contains(., "Alle "))]')
...
def process_product(data, context, session):
    ...
    product.ssid = data.xpath('//meta[@itemprop="gtin13"]/@content').string()
    if not product.ssid:
