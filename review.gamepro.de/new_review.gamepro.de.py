...
def run(context, session):
    ...
    session.queue(Request('https://www.gamepro.de/tests/', force_charset='utf-8'), process_revlist, dict())
...
def process_revlist(data, context, session):
    ...
    if revs:
        ...
        next_url = 'https://www.gamepro.de/tests/{}'.format(next_page)
        session.queue(Request(next_url, force_charset='utf-8'), process_revlist, dict(page=next_page))
...
def process_review(data, context, session):
    ...
    product.name = data.xpath('//div[@class="media-body"]/p/a/text()').string()
    if not product.name:
        product.name = context['title'].split(' im Test')[0].split(' - Test')[0].split(' -Test')[0].replace(' - Review', '').replace('Anime-Review: ', '').replace(' - Beta-Test', '').strip()
    ...
    author = author.xpath('//p[contains(@class, "publication-info")]/*[normalize-space(.)]//text()').string()
    author_url = author.xpath('//p[contains(@class, "publication-info")]/a/@href').string()
    ...
    if next_url:
        ...
        session.do(Request(next_url, force_charset='utf-8'), process_review_next, dict(excerpt=excerpt, review=review, product=product))
...
def process_review_next(data, context, session):
    ...
    next_url = data.xpath('//link[@rel="next"]/@href').string()
    if next_url:
        session.do(Request(next_url, force_charset='utf-8',), process_review_next, dict(context, review=review, page=page+1))
