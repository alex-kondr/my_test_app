...
def run(context: dict[str, str], session: Session):
    session.queue(Request('https://nettbutikk.ice.no/mobiltelefoner', force_charset='utf-8'), process_catlist, dict())
...
def process_prodlist(data: Response, context: dict[str, str], session: Session):
    prods = data.xpath('(//ul[@data-analytics-id="category-products"])[1]/li[a[contains(@class, "product-card")]]')
    for prod in prods:
