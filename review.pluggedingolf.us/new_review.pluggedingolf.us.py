...
def process_revlist(data, context, session):
    ...
    for rev in revs:
        ...
        session.queue(Request(url, use='curl', force_charset='utf-8', options=OPTIONS.format(time1=time1, time2=time1+40)), process_review, dict(context, title=title, url=url))

    next_url = data.xpath('//link[@rel="next"]/@href').string()
    if not next_url and not context.get('restart1'):
        time.sleep(100)
        time1 = time.time()
        session.do(Request(data.response_url, use='curl', force_charset='utf-8', max_age=0, options=OPTIONS.format(time1=time, time2=time1+40)), process_revlist, dict(context, restart1=True))
...
def process_review(data, context, session):
    if not data.xpath('//div[@class="elementor-widget-container" and h2]/p') and not context.get('restart'):
        time.sleep(100)
        time1 = time.time()
        session.do(Request(data.response_url, use='curl', force_charset='utf-8', max_age=0, options=OPTIONS.format(time1=time, time2=time1+40)), process_review, dict(context, restart=True))
