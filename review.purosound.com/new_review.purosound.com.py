...
def run(context: dict[str, str], session: Session):
    session.queue(Request('https://purosound.com/', use='curl', force_charset='utf-8'), process_frontpage, dict())
...
def process_product(data: Response, context: dict[str, str], session: Session):
    ...
    session.do(Request(revs_url, max_age=0, use='curl', force_charset='utf-8'), process_reviews, dict(product=product))
...
def process_reviews(data: Response, context: dict[str, str], session: Session):
    ...
    if revs_cnt and int(revs_cnt) > offset:
        ...
        session.do(Request(next_url, use='curl', force_charset='utf-8', max_age=0), process_reviews, dict(product=product, revs_cnt=revs_cnt, offset=offset))
