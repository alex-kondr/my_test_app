...
def run(context: dict[str, str], session: Session):
    ...
    session.queue(Request('https://www.storedj.com.au/', force_charset='utf-8', use='curl', max_age=0), process_frontpage, dict())
...
def process_prodlist(data: Response, context: dict[str, str], session: Session):
    ...
    prods_cnt = context.get('prods_cnt', prods_json.get('found'))
    ...
    if offset < prods_cnt:
        ...
        session.do(Request(next_url, use='curl', options=options, force_charset='utf-8', max_age=0), process_prodlist, dict(context, page=next_page, offset=offset, prods_cnt=prods_cnt))
