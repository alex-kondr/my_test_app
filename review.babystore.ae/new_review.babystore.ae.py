...
def run(context, session):
    ...
    session.queue(Request('https://www.babystore.ae/', use='curl', force_charset='utf-8'), process_frontpage, dict())


def process_frontpage(data, context, session):
    strip_namespace(data)

    cats = data.xpath('//li[contains(@class, "menu-item")]')
    for cat in cats:
...
def process_revlist(data, context, session):
    ...
    revs = data.xpath('//div[@id="productList"]/div')
    for rev in revs:
        name = rev.xpath('.//div[contains(@class, "product-content")]/h2/a//text()').string()
        url = rev.xpath('.//div[contains(@class, "product-content")]/h2/a/@href').string()
