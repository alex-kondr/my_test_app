...
def process_review(data, context, session):
    product = Product()
    product.name = context['title'].replace(' Review', '').split(' | ')[0].replace(' Tested and Rated', '').replace(' review', '').replace('Review of ', '').replace('Review: ', '').strip(' .:')
    product.ssid = context['url'].split('/')[-2].replace('-review', '').replace('review-', '')

    product.url = data.xpath('//a[contains(., "Shop")]/@href[not(contains(., "sectionhiker.com"))]').string()
    if not product.url:
    ...
    summary = data.xpath('//h2[contains(@class, "title")]//text()').string(multiple=True)
    if summary:
    ...
    summary_2 = data.xpath('//div[@class="review-short-summary"]/p//text()').string()

    if conclusion and not summary and summary_2:
        review.add_property(type='summary', value=summary_2)

    if not conclusion and summary_2:
        review.add_property(type='conclusion', value=summary_2)

    excerpt = data.xpath('(//h2|//h3)[regexp:test(., "Recommendation|The Bottom Line")]/preceding-sibling::p//text()').string(multiple=True)
    if not excerpt:
        excerpt = data.xpath('//div[contains(@class, "entry-content")]/p[not(contains(., "Disclosure:"))]//text()').string(multiple=True)
