...
def run(context: dict[str, str], session: Session):
    ...
    session.queue(Request('https://www.thephoblographer.com/reviews/', force_charset='utf-8'), process_catlist, dict())
...
def process_review(data: Response, context: dict[str, str], session: Session):
    ...
    excerpt = data.xpath('//div[@class="entry-content"]/p[not(@class or contains(., "Also, please follow us") or starts-with(normalize-space(text()), "–") or preceding-sibling::h2[contains(., "Conclusion")])]//text()[not(regexp:test(., "Amazon.com Widgets|Pros:"))]').string(multiple=True)
    if excerpt:
