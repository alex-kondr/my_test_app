...
def run(context, session):
    ...
    session.queue(Request("https://www.consobaby.co.uk", use='curl'), process_frontpage, dict())
            ...
            for sub_cat in sub_cats:
                ...
                if sub_cats1:
                    for sub_cat1 in sub_cats1:
                        ...
                        session.queue(Request(url + '?dir=desc&order=review', use='curl'), process_prodlist, dict(cat=name + '|' + sub_name + '|' + sub_name1))
                else:
                    url = sub_cat.xpath('a/@href').string()
                    session.queue(Request(url + '?dir=desc&order=review', use='curl'), process_prodlist, dict(cat=name + '|' + sub_name ))
...
def process_reviews(data,context, session):
    ...
    for rev in revs:
        ...
        for grade in grades:
            ...
            grade_value = grade.xpath(".//span[@class='value']/svg//@name").string().split("stars-")[-1]
    ...
    if next_url:
        session.do(Request(next_url, use='curl'), process_reviews, dict(context, product=product))
