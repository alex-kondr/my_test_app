...
def process_frontpage(data, context, session):
    ...
    for cat in cats:
        ...
        if name not in XCAT:
            cats1 = cat.xpath('div[@class="child"]/ul/li')
            for cat1 in cats1:
                cat1_name = cat1.xpath('a/span[not(@class)]/text()').string().title()

                subcats = cat1.xpath('ul/li/a')
                if subcats:
                    for subcat in subcats:
                        subcat_name = subcat.xpath('span[not(@class)]/text()').string()
                        url = subcat.xpath("@href").string()
                        session.queue(Request(url + '?dir=desc&order=review'), process_prodlist, dict(cat=name + '|' + cat1_name + '|' + subcat_name))
                else:
                    url = cat1.xpath('a/@href').string()
                    session.queue(Request(url + '?dir=desc&order=review'), process_prodlist, dict(cat=name + '|' + cat1_name))
