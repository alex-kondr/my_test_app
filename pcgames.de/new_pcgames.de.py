...
def process_review(data, context, session):
    ...
    product.name = data.xpath('//span[contains(@class, "productTitle")]//text()').string(multiple=True) or context['title'].split(': Blu-ray-Test ')[0].replace(' im Blu-ray-Test', '').replace(' - Review/Filmkritik', '').replace(' --- Filmkritik / Review', '').replace(' (Review / Filmkritik)', '').replace(' (Review/Filmkritik)', '').replace(' (Review/Fimkritik)', '').replace('(Filmkritik/Review)', '').replace(' - Kinokritik/Review', '').replace(' - Filmkritik/Review', '').replace(' (Kinokritik/Review)', '').replace(' im Test', '').replace('Review: ', '').replace(' - Kinoreview & Trailer', '').replace(' - Großes Review', '').replace(' - Review', '').replace(' (Kinokritik / Review)', '').replace(' (Review/Kinokritik)', '').replace(': Kinokritik/Review', '').repalce(' (Kino-Kritik/Review)', '').replace(' (Blu-ray-Kritik / Review)', '').replace(' (Kinokritik/ Review)', '').replace(' (Kritik / Review)', '').strip()
    ...
    if pages:
        ...
        excerpt = context['excerpt'].replace(u'\x96', '').replace(u'\x92', '').strip()
