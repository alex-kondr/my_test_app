...
def run(context, session):
    session.queue(Request("https://milex.co.za/", max_age=0), process_catlist, dict())
...
def process_product(data, context, session):
    ...
    session.do(Request(revs_url + '1&per_page=5&product_id=' + product.ssid, max_age=0), process_reviews, dict(product=product, revs_url=revs_url))
...
def process_reviews(data, context, session):
    ...
    if new_data.xpath('//a[contains(., "Load More")]/text()').string():
        ...
        session.do(Request(context["revs_url"] + str(page) + "&per_page=5&product_id=" + product.ssid, max_age=0), process_reviews, dict(context, product=product, page=page))
