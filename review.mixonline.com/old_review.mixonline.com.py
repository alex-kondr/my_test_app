...
def run(context: dict[str, str], session: Session):
    ...
    session.queue(Request('https://www.mixonline.com/technology/reviews', use='curl', max_age=0), process_revlist, dict())
...
def process_review(data: Response, context: dict[str, str], session: Session):
    ...
    product.ssid = data.xpath('//script/@data-post').string()
...
', '').strip()
    ...
    if conclusion:
        conclusion = conclusion.replace(u'\uFEFF', '').replace(u'
', '').strip()
    ...
    if excerpt:
        excerpt = excerpt.replace(u'\uFEFF', '').replace(u'
', '').strip()
