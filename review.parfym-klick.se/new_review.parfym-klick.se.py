...
def run(context, session):
    ...
    session.queue(Request('https://www.parfym-klick.se/', force_charset='utf-8', use='curl'), process_frontpage, dict())
...
def process_frontpage(data, context, session):
    ...
    for cat in cats:
        ...
        if name not in XCAT:
            session.queue(Request(url + '?l=144', force_charset='utf-8', use='curl'), process_catlist, dict(cat=name))
...
def process_catlist(data, context, session):
    ...
    for sub_cat in sub_cats:
        ...
        session.queue(Request(url, force_charset='utf-8', use='curl'), process_prodlist, dict(cat=context['cat'] + '|' + sub_name))
    else:
...
def process_prodlist(data, context, session):
    ...
    for prod in prods:
        ...
        session.queue(Request(url, force_charset='utf-8', use='curl'), process_product, dict(context, url=url))
