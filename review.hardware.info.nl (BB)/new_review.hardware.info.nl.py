...
def run(context, session):
    session.queue(Request('https://hardware.info/reviews.html'), process_revlist, dict())
...
def process_review(data, context, session):
    ...
    if next_page:
        ...
        session.queue(Request(next_url), process_review_next, dict(excerpt=excerpt, review=review, product=product))
...
def process_review_next(data, context, session):
    ...
    if block:
        time.sleep(600)
        session.do(Request(data.response_url, max_age=0), process_review_next, dict(context))
        return
    ...
    if next_page:
        ...
        session.queue(Request(url), process_review_next, dict(context, review=review))
