...
def run(context, session):
    ...
    session.queue(Request('https://www.mobiflip.de/thema/testberichte/', force_charset='utf-8'), process_revlist, dict())
...
def process_review(data, context, session):
    ...
    conclusion = data.xpath('//h2[contains(., "Fazit") or contains(., "fazit")]/following-sibling::p[not(.//span[@class="sharep-short-star"])]//text()[not(contains(., "tl;dr"))]').string(multiple=True)
    if conclusion:
        review.add_property(type='conclusion', value=conclusion)

    excerpt = data.xpath('(//div[@class="content-short"]/p|//div[@class="content-short"]/ul/li)[not(preceding::h2[contains(., "Fazit") or contains(., "fazit")] or .//span[@class="sharep-short-star"])]//text()[not(contains(., "tl;dr"))]').string(multiple=True)
    if excerpt:
