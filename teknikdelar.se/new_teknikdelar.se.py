...
def process_product(data, context, session):
    ...
    manufacturer = data.xpath('(//tr[th[contains(., "Märke")]]/td|//li[contains(., "Märke")])/text()').string()
    if manufacturer:
        product.manufacturer = manufacturer.replace('Märke: ', '').strip()
    ...
    revs_cnt = int(data.xpath('//span[@class="review-text"]/text()').string().replace('Omdömen', '').strip('( )').replace(' ', ''))
