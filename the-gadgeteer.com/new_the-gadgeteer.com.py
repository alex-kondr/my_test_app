...
def process_review(data, context, session):
    ...
    product.ssid = context['url'].split('/')[-2].replace('_review', '')
    product.category = data.xpath('//span[@class="ast-terms-link"]/a[contains(@href, "https://the-gadgeteer.com/tag/")]/text()').string()
    ...
    pros = data.xpath('(//h2[contains(., "What I like")]/following-sibling::ul)[1]/li')
    if not pros:
    ...
    cons = data.xpath('(//h2[contains(., "What needs")]/following-sibling::ul)[1]/li')
    if not cons:
    ...
    if summary:
        summary = summary.replace('REVIEW', '').strip(' –')
        review.add_property(type='summary', value=summary)

    conclusion = data.xpath('//h2[regexp:test(., "Final thoughts|Conclusion")]/following-sibling::p[not(@class or regexp:test(., ".+:.+\+"))]//text()').string(multiple=True)
    if conclusion:
        review.add_property(type='conclusion', value=conclusion)

    excerpt = data.xpath('//h2[regexp:test(., "Final thoughts|Conclusion")]/preceding-sibling::p[not(@class or strong[contains(., "REVIEW")] or regexp:test(., ".+:.+\+"))]//text()').string(multiple=True)
    if not excerpt:
        excerpt = data.xpath('//div[contains(@class, "content")]/p[not(@class or strong[contains(., "REVIEW")] or regexp:test(., ".+:.+\+"))]//text()').string(multiple=True)
