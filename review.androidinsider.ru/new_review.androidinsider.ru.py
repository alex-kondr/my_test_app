...
def process_revlist(data, context, session):
    ...
    next_page = data.xpath('//a[contains(@class, "next")]/@href').string()
    if next_page:
...
def process_product(data, context, session):
    ...
    product.category = data.xpath('//a[@rel="tag" and not(contains(., "Распродажа") or contains(., "АлиЭкспресс"))]//text()').string()
    ...
    if not excerpt:
        excerpt = data.xpath('//h2[contains(., "Что лучше")][last()]/preceding-sibling::p[not(a[@class="link-alert"] or code)]//text()').string(multiple=True)
    if not excerpt:
