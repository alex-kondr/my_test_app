...
def run(context, session):
    ...
    session.queue(Request("http://androidinsider.ru/", use="curl", max_age=0), process_frontpage, dict())
...
def process_revlist(data, context, session):
    ...
    if next_page:
        session.queue(Request(next_page, use="curl", max_age=0), process_revlist, dict(context))
