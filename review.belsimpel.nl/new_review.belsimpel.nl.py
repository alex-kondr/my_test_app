...
XCAT = ['Weekdeals', 'Telefoon met abonnement', 'Sim Only', 'Verlengen', 'Klantenservice', 'Bizarre Belsimpel Dagen', 'Prijsbrekers']
...
def process_get_token(data: Response, context: dict[str, str], session: Session):
...
def process_prodlist(data: Response, context: dict[str, str], session: Session):
    prods = data.xpath('//div[contains(@class, "item__title-rating")]')
    for prod in prods:
        name = prod.xpath('.//a/text()').string()
        url = prod.xpath('.//a/@href').string()

        revs_cnt = prod.xpath('.//div[contains(@class, "title-rating")]//span[regexp:test(., "^\(\d+\)")]/text()[not(contains(., "Geen beoordelingen"))]').string()
        if revs_cnt:
...
def process_reviews(data: Response, context: dict[str, str], session: Session):
    ...
    if revs_json.get('status') != 'success' and not context.get('repeated'):
        token = session.do(Request('https://www.belsimpel.nl/API/vergelijk/Exchange?response_type=token&client_id=nl.belsimpel.public.web&scope=Reviews', max_age=0), process_get_token, dict())
        revs_url = 'https://www.belsimpel.nl/API/Reviews/v1.0/ProductReviews/{}?locale=nl_NL'.format(product.ssid)
        options = "--compressed -H 'Accept-Encoding: deflate' -H 'Authorization: Bearer " + token + "'"
        session.do(Request(revs_url, use='curl', force_charset='utf-8', options=options, max_age=0), process_reviews, dict(context, token=token, repeated=True))
        return
