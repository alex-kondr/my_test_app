...
def run(context: dict[str, str], session: Session):
    ...
    session.queue(Request('https://overclockers.ru/lab', max_age=0, force_charset='utf-8', use='curl'), process_catlist, dict())
...
def process_catlist(data: Response, context: dict[str, str], session: Session):
    ...
    for cat in cats:
        ...
        if name not in XCAT:
            session.queue(Request(url, max_age=0, force_charset='utf-8', use='curl'), process_revlist, dict(cat=name))
...
def process_revlist(data: Response, context: dict[str, str], session: Session):
    ...
    for rev in data.xpath("//div[@class='item article-wrap']/div[@class='content']"):
        ...
        session.queue(Request(url, max_age=0, force_charset='utf-8', use='curl'), process_review, dict(context, title=title, url=url))

    next = data.xpath("//div[@class='ui pagination menu']/a[@class='item next']/@href").string()
    if next:
        session.queue(Request(next, max_age=0, force_charset='utf-8', use='curl'), process_revlist, dict(context))
...
def process_review(data: Response, context: dict[str, str], session: Session):
    ...
    last_page = ''
    pages = data.xpath('(//div[@class="ui list"])[1]//div[@class="content"]')
    for page in pages:
    ...
    if last_page:
        session.do(Request(last_page, max_age=0, force_charset='utf-8', use='curl'), process_lastpage, dict(product=product, review=review, excerpt=excerpt))
...
def process_lastpage(data: Response, context: dict[str, str], session: Session):
    strip_namespace(data)
