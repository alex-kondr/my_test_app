...
def run(context: dict[str, str], session: Session):
    session.queue(Request('http://www.pcgamer.com/reviews/'), process_revlist, dict(cat='Games|PC'))
    session.queue(Request('https://www.pcgamer.com/hardware/reviews/'), process_revlist, dict(cat='Hardware'))
...
def process_revlist(data: Response, context: dict[str, str], session: Session):
    ...
    session.queue(Request(next_url), process_revlist, dict(context, page=page+1))
...
def process_review(data: Response, context: dict[str, str], session: Session):
    ...
    summary = data.xpath('//div[h1]/h2//text()').string(multiple=True)
    if summary:
