...
def process_review(data, context, session):
    ...
    product.name = data.xpath('//div[@class="aawp-product__thumb" and not(preceding-sibling::*[contains(., "Bestseller")])]/a/@title').string()
    if not product.name:
    ...
    product.url = data.xpath('//div[@class="aawp-product__thumb" and not(preceding-sibling::*[contains(., "Bestseller")])]/a/@href').string()
    if not product.url:
        product.url = context['url']

    product.category = data.xpath('//a[@rel="tag" and not(contains(., "Test"))]/text()').string()
    ...
    excerpt = data.xpath('(//h2|//h3)[contains(., "Fazit")][last()]/preceding-sibling::p[not(.//*[contains(@style, "color") or contains(@style, "decoration")])]//text()').string(multiple=True)
    if not excerpt:
        excerpt = data.xpath('(//h2|//h3)[contains(., "Fazit")][last()]/preceding::p[not(.//*[contains(@style, "color") or contains(@style, "decoration")] or contains(., "Fazit:"))]//text()').string(multiple=True)
