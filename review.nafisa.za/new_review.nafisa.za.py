...
def run(context, session):
    session.queue(Request('https://www.recharged.co.za/review/', force_charset='utf-8'), process_revlist, dict())
...
def process_review(data, context, session):
    ...
    excerpt = data.xpath('//div[contains(@class, "post-single-content")]/p[not(preceding-sibling::h2[contains(., "Final Verdict")])]//text()').string(multiple=True)
    if excerpt:
