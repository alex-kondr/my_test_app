...
def run(context: dict[str, str], session: Session):
    ...
    session.queue(Request('https://www.soccercleats101.com/category/cleat-reviews/', force_charset='utf-8', use='curl'), process_revlist, dict())
...
def process_review(data: Response, context: dict[str, str], session: Session):
    ...
    if not conclusion:
        conclusion = data.xpath('//p[strong[regexp:test(., "Summary", "i")]]//text()[not(regexp:test(., "Summary", "i"))]').string(multiple=True)
