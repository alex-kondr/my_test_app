...
URLS = [
    'https://www.latercera.com/pf/api/v3/content/fetch/story-feed-sections-fetch?query={%22feedOffset%22:offset,%22feedSize%22:24,%22fromComponent%22:%22result-list%22,%22includeSections%22:%22/mouse%22,%22query%22:%22/mouse%22,%22tagSlug%22:%22/mouse%22}&d=1247&mxId=00000000&_website=la-tercera',
    'https://www.latercera.com/pf/api/v3/content/fetch/story-feed-sections-fetch?query={%22feedOffset%22:offset,%22feedSize%22:5,%22fromComponent%22:%22result-list%22,%22includeSections":%22/tecnologia%22,%22query%22:%22/tecnologia%22,%22tagSlug%22:%22/tecnologia%22}&d=1247&mxId=00000000&_website=la-tercera'
]
...
def run(context: dict[str, str], session: Session):
    ...
    for url in URLS:
        session.queue(Request(url.replace('offset', '0'), force_charset='utf-8', max_age=0), process_revlist, dict(cat_url=url))
...
def process_revlist(data: Response, context: dict[str, str], session: Session):
    ...
    for rev in revs:
        ...
        if 'reseña' in title.lower() or 'review' in title.lower():
            session.queue(Request(url, force_charset='utf-8', max_age=0), process_review, dict(url=url))

    revs_cnt = context.get('revs_cnt', revs_json.get('count', 0))
    offset = context.get('offset', 0) + 24
    if offset < revs_cnt:
        session.queue(Request(context['cat_url'].replace('offset', str(offset)), force_charset='utf-8', max_age=0), process_revlist, dict(context, revs_cnt=revs_cnt, offset=offset))
