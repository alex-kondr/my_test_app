...
def process_review(data: Response, context: dict[str, str], session: Session):
    ...
    if not name or name == 'Film':
        name = data.xpath('//*[self::p or self::div][contains(text(), "Wertung für")]//text()').string(multiple=True) or context['title']
        name = name.split('Wertung für')[-1].split('Review zu ')[-1].split('Hardware-Test: ')[-1]
    ...
    product.ssid = product.url.split('/review/')[0].split('/')[-1]
    ...
    for author in authors:
        author_name = serialize_text(author.xpath('text()').string())
...
def process_review_next(data: Response, context: dict[str, str], session: Session):
    ...
    if grade_overall and grade_overall.strip(' -'):
    ...
    if next_url:
        ...
        if context.get('summary'):
            summary = serialize_text(context['summary']).strip()
            review.add_property(type='summary', value=summary)

        if context.get('conclusion'):
            conclusion = serialize_text(context['conclusion']).replace('amp;', '').replace('lt;', '').replace('/ifr', '').strip()
            if conclusion:
                review.add_property(type='conclusion', value=conclusion)

        if context.get('excerpt'):
            excerpt = serialize_text(context['excerpt']).replace('amp;', '').strip()
