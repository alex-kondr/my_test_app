...
def run(context: dict[str, str], session: Session):
    ...
    session.queue(Request('https://de.ign.com/article/review'), process_category, {})
...
def process_category(data: Response, context: dict[str, str], session: Session):
    ...
    for cat in cats:
        ...
        url = 'https://de.ign.com/article/review?keyword__type=' + cat.xpath('@class').string()
...
def process_review(data: Response, context: dict[str, str], session: Session):
    ...
    if platforms:
        product.category = 'Spellen|' + platforms.replace('-', ' ').upper().strip()
    ...
    conclusion = data.xpath('//p[preceding-sibling::h3[regexp:test(., "fazit", "i")]]/text()').string(multiple=True)
    if not conclusion:
        conclusion = data.xpath('//p[preceding::div[contains(., "Schlussgedanken")]][not(contains(., "IGN-Wertung für") or contains(., "Schlussgedanken") or contains(., "Lest auf der nächsten Seite") or contains(., "Auf der letzten Seite") or contains(., "amp;"))][not(ancestor::div[@id="ratingsBox"] or preceding::div[@id="ratingsBox"])]//text()').string(multiple=True)
    if not conclusion:
        conclusion = data.xpath('//div[@id="id_text"]//text()[not(contains(., "IGN-Wertung für") or contains(., "Schlussgedanken") or contains(., "Lest auf der nächsten Seite") or contains(., "Auf der letzten Seite") or contains(., "amp;"))][not(ancestor::div[@id="ratingsBox"] or preceding::div[@id="ratingsBox"])][preceding::div[contains(text(), "Schlussgedanken")]]').string(multiple=True)
    if not conclusion:
    ...
    context['excerpt'] = data.xpath('//div[@id="id_text"]/p[not(preceding::div[contains(text(), "Schlussgedanken")])]//text()[not(contains(., "Schlussgedanken") or contains(., "Lest auf der nächsten Seite") or contains(., "Auf der letzten Seite") or contains(., "Wertung für"))]').string(multiple=True)
    if not context['excerpt']:
        context['excerpt'] = data.xpath('//div[@id="id_text"]//text()[not(ancestor::div[contains(text(), "Schlussgedanken")] or preceding::div[contains(text(), "Schlussgedanken")])][not(contains(., "Schlussgedanken") or contains(., "Lest auf der nächsten Seite") or contains(., "Auf der letzten Seite") or contains(., "Wertung für"))]').string(multiple=True)
...
def process_review_last(data: Response, context: dict[str, str], session: Session):
    ...
    if context.get('page'):
        ...
        excerpt = data.xpath('//div[@id="id_text"]/p[not(preceding::div[contains(text(), "Schlussgedanken")])]//text()[not(contains(., "Schlussgedanken") or contains(., "Lest auf der nächsten Seite") or contains(., "Auf der letzten Seite") or contains(., "Wertung für"))]').string(multiple=True)
        if not excerpt:
            excerpt = data.xpath('//div[@id="id_text"]//text()[not(ancestor::div[contains(text(), "Schlussgedanken")] or preceding::div[contains(text(), "Schlussgedanken")])][not(contains(., "Schlussgedanken") or contains(., "Lest auf der nächsten Seite") or contains(., "Auf der letzten Seite") or contains(., "Wertung für"))]').string(multiple=True)
