...
def run(context: dict[str, str], session: Session):
    ...
    session.queue(Request('https://www.gamereactor.de/'), process_frontpage, dict())
    session.queue(Request('https://www.gamereactor.de/Kritiken/'), process_revlist, dict())
    session.queue(Request('https://www.gamereactor.de/Film/'), process_revlist, dict(cat='Film'))
    session.queue(Request('https://www.gamereactor.de/series/'), process_revlist, dict(cat='Series'))
...
def process_frontpage(data: Response, context: dict[str, str], session: Session):
    ...
    for cat in cats:
        ...
        if name not in XCAT:
            session.queue(Request(url, force_charset='utf-8', max_age=0), process_revlist, dict(cata='Hardware|' + name))
...
def process_review(data: Response, context: dict[str, str], session: Session):
    ...
    product.manufacturer = data.xpath('//div[contains(@class, "side-sticky")]//div[contains(span, "Desarrollador")]/span[not(contains(., "Desarrollador"))]/text()').string(multiple=True)

    title = data.xpath("//h1/text()").string()
    product.name = data.xpath('//div[@class="gr-headgrid"]//div[normalize-space(span/text())="Análisis"]/span[not(contains(., "Análisis"))]/text()').string()
    if not product.name:
        product.name = title.replace('Análisis de ', '').replace('...', '').strip()

    platforms = data.xpath('//div[contains(span, "Plataformas")]/div/span/text()').join('/')
    if platforms:
        product.category += '|' + platforms

    genres = data.xpath('//div[contains(@class, "side-sticky")]//div[contains(span, "Género")]/span[not(contains(., "Género"))]/text()').join('/')
    if genres:
    ...
    if excerpt:
        excerpt = re.sub(r'<.*?>', '', excerpt).replace('<bild<', '').replace('" target="_blank">', '').replace(u'\uFEFF', '').replace('&#363;', u'ū').replace('&#333;', u'ō').strip()
