...
def run(context, session):
    ...
    session.queue(Request('https://www.hi-fi.com.pl/', max_age=0, force_charset='utf-8'), process_frontpage, dict())
...
def process_frontpage(data, context, session):
    ...
    for cat in cats:
        ...
        session.queue(Request(url, max_age=0, force_charset='utf-8'), process_revlist, dict(cat=name))
...
def process_revlist(data, context, session):
    ...
    for rev in revs:
        ...
        session.queue(Request(url, max_age=0, force_charset='utf-8'), process_review, dict(context, title=title, author=author, author_url=author_url, url=url))
    ...
    if next_url:
        session.queue(Request(next_url, max_age=0, force_charset='utf-8'), process_revlist, dict(context))
