...
def run(context, session):
    ...
    session.queue(Request('https://www.gamereactor.de/', use='curl', force_charset='utf-8', max_age=0), process_frontpage, dict())
    session.queue(Request('https://www.gamereactor.de/Kritiken/', use='curl', force_charset='utf-8', max_age=0), process_revlist, dict())
    session.queue(Request('https://www.gamereactor.de/Film/', use='curl', force_charset='utf-8', max_age=0), process_revlist, dict(cat='Film'))
    session.queue(Request('https://www.gamereactor.de/series/', use='curl', force_charset='utf-8', max_age=0), process_revlist, dict(cat='Series'))
...
def process_frontpage(data, context, session):
    ...
    for cat in cats:
        ...
        if name not in XCAT:
            session.queue(Request(url, use='curl', force_charset='utf-8', max_age=0), process_revlist, dict(cata='Hardware|' + name))
