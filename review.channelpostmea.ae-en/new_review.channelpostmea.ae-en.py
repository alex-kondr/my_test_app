...
def process_review(data, context, session):
    ...
    product.category = data.xpath('//a[@rel="category tag" and not(regexp:test(., "Review|News|Articles|Products|Newsletter"))]/text()').join('|')
    ...
    if conclusion:
        conclusion = conclusion.split('Price:')[0].split('Comments')[0].strip()
