...
def process_review(data, context, session):
    ...
    if not conclusion:
        conclusion = data.xpath('//h4[contains(., "Verdict")]/following-sibling::p[not(preceding-sibling::hr or contains(., "Reviewed on "))]//text()').string(multiple=True)
    if not conclusion:
    ...
    if not excerpt:
        excerpt = data.xpath('//div[contains(@class, "article_body_conten")]/p[not(preceding-sibling::hr or contains(., "Reviewed on "))]//text()').string(multiple=True)
