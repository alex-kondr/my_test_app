...
def process_revlist(data, context, session):
    ...
    for rev in revs:
        ...
        session.queue(Request(url, use='curl', max_age=0, options=options), process_review, dict(title=title, url=url))
...
def process_review(data, context, session):
    ...
    excerpt = data.xpath('(//p[strong[regexp:test(., "verdict", "i")]]/preceding-sibling::p[not(strong[regexp:test(., "Pros|Cons")] or regexp:test(., "Rating:|Click here for"))]//text()[not(contains(., "Review:") or regexp:test(., "\d.?\d?/\d") or contains(., "Price:"))]|//p[strong[regexp:test(., "verdict", "i")]]//text()[not(regexp:test(., "verdict", "i"))])[not(starts-with(normalize-space(.), "-"))]').string(multiple=True)
    if not excerpt:
        excerpt = data.xpath('(//p[strong[regexp:test(., "conclusion", "i")]]/preceding-sibling::p[not(strong[regexp:test(., "Pros|Cons")] or regexp:test(., "Rating:|Click here for"))]//text()[not(contains(., "Review:") or regexp:test(., "\d.?\d?/\d") or contains(., "Price:"))]|//p[strong[regexp:test(., "conclusion", "i")]]//text()[not(regexp:test(., "conclusion", "i"))])[not(starts-with(normalize-space(.), "-"))]').string(multiple=True)
    if not excerpt:
