...
def process_review(data, context, session):
    product = Product()
    product.name = context['title'].replace(': A Preview', '').replace(' Preview', '').replace(' Review', '').replace(' review', '').replace(' Tested', '').strip()
    product.url = context['url']
    product.ssid = product.url.split('/')[-2]

    product.category = data.xpath('//div[nav[@id="crumbs"]]/a[not(@id or contains(., "Reviews") or contains(., "Home"))]//text()[normalize-space(.)]').string()
    ...
    excerpt = data.xpath('//div[@class="entry"]/div[@data-id]/p//text()').string(multiple=True)
    if not excerpt:
    ...
    if pages:
        ...
        session.do(Request(page_url, use='curl'), process_review_last, dict(excerpt=excerpt, review=review, product=product, pages=True))
...
def process_review_last(data, context, session):
    ...
    pros = data.xpath("(//p[contains(strong, 'Pro')]/following-sibling::ul)[1]/li")
    for pro in pros:
    ...
    cons = data.xpath("(//p[contains(strong, 'Con')]/following-sibling::ul)[1]/li")
    for con in cons:
