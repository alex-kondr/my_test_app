...
def run(context: dict[str, str], session: Session):
    session.queue(Request('https://www.wintotal.de/'), process_frontpage, dict())
...
def process_review(data: Response, context: dict[str, str], session: Session):
    product = Product()
    product.name = context['title'].replace('Angetestet: ', '').split(' » ')[0].split(' – ')[0].split(' Test: ')[0].split(': ')[0].replace(' im Test', '').replace('Test von ', '').replace(' Test und Ratgeber', '').strip()
    ...
    if not pros:
        pros = data.xpath('//p[normalize-space(strong/text())="Vorteile:"]/strong[not(contains(., "Vorteile:"))]')
        for pro in pros:
            pro = pro.xpath('.//text()|following-sibling::text()[1]').string(multiple=True)
            if pro:
                pro = pro.strip(' +-*.;•–')
                if len(pro) > 1:
                    review.add_property(type='pros', value=pro)
    ...
    if not cons:
        cons = data.xpath('//p[normalize-space(strong/text())="Nachteile:"]/strong[not(contains(., "Nachteile:"))]')
        for con in cons:
            con = con.xpath('.//text()|following-sibling::text()[1]').string(multiple=True)
            if con:
                con = con.strip(' +-*.;•–')
                if len(con) > 1:
                    review.add_property(type='cons', value=con)
    ...
    if not excerpt:
        excerpt = data.xpath('//div[contains(@class, "content")]//p[not(@class or contains(., "Hinweis:") or normalize-space(strong/text())="Nachteile:" or normalize-space(strong/text())="Vorteile:") or contains(@class, "schema")]//text()').string(multiple=True)
