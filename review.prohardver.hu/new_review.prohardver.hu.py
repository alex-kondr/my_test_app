...
def process_review(data, context, session):
    ...
    cats = data.xpath('//ol[@class="breadcrumb"]/li[not(regexp:test(., "Tesztek|Hazai Pálya"))]//text()[normalize-space()]').strings()
    if len(cats) > 1:
    ...
    excerpt = data.xpath('//p[b[contains(., "Összefoglalás")]]/preceding-sibling::p//text()').string(multiple=True)
    if not excerpt:
        excerpt = data.xpath('//div[@class="content-body"]/p[not(b[regexp:test(., "pozitívumai|hiányosságai")])]//text()').string(multiple=True)
