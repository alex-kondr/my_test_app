...
def run(context: dict[str, str], session: Session):
    ...
    session.queue(Request('https://www.multicom.no/', force_charset='utf-8', use='curl'), process_frontpage, {})
                                ...
                                if cats4:
                                    for cat4 in cats4:
                                        ...
                                        if name4 and name4 not in XCAT:
                                            ...
                                            session.queue(Request(url, force_charset='utf-8', use='curl'), process_category, dict(cat=name1+'|'+name2+'|'+name3+'|'+name4))

                                elif name1 and name2 and name3:
                                    url = cat3.xpath('a/@href').string()
                                    session.queue(Request(url, force_charset='utf-8', use='curl'), process_category, dict(cat=name1+'|'+name2+'|'+name3))

                    elif name1 and name2:
                        url = cat2.xpath('a/@href').string()
                        session.queue(Request(url, force_charset='utf-8', use='curl'), process_category, dict(cat=name1+'|'+name2))
...
def process_category(data: Response, context: dict[str, str], session: Session):
    ...
    if next_url:
        session.queue(Request(next_url, force_charset='utf-8', max_age=0, use='curl'), process_category, dict(context))
