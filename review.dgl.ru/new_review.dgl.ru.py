...
def run(context, session):
    ...
    session.queue(Request('https://www.dgl.ru/reviews', force_charset='utf-8'), process_revlist, dict())


def process_revlist(data, context, session):
    time.sleep(random.uniform(1, 3))

    revs = data.xpath('//a[@class="result-item" or @class="box"]')
    for rev in revs:
        title = rev.xpath('@title').string()
        url = rev.xpath('@href').string()

        if any(title_in in title.lower() for title_in in TITLE):
...
def process_review(data, context, session):
    ...
    product = Product()
    product.name = context['title'].replace('Предварительный обзор:', '').replace('Предварительный обзор', '').replace('Обзор смартфона', '').replace('Stuff-обзор:', '').replace('Взгляд от Stuff:', '').replace('WHF-обзор:', '').replace('Взгляд журнала Stuff:', '').replace('Обзор наушников', '').replace('Мини-обзор:', '').replace('Блиц-обзор', '').replace('Обзор:', '').split(':')[0].split(' - ')[0].split(' — ')[0].strip()
    ...
    author = data.xpath('//div[@class="article-author-name" and not(.//span[normalize-space(text())="admin"])]//text()').string(multiple=True)
    author_url = data.xpath('//div[@class="article-author-name" and not(.//span[normalize-space(text())="admin"])]//a/@href').string()
