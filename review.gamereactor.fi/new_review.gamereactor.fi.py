...
def process_review(data, context, session):
    ...
    product.manufacturer = data.xpath('//li[span[contains(., "Valmistaja:") or contains(., "Kehittäjä:") or contains(., "Elokuvayhtiö:")]]//text()[not(parent::span)]').string(multiple=True) or data.xpath('//li[span[contains(., "Julkaisija:")]]//text()[not(parent::span)]').string(multiple=True)
    product.url = context['url']
    product.ssid = data.xpath('//article/@data-id').string()

    category = context['cat']
    plats = data.xpath('//li[span[contains(., "Testattu:") or contains(., "Yhteensopiva:")]]/text()').string(multiple=True)
    if plats:
        plats = plats.split(',')
        for plat in plats:
    ...
    if excerpt:
        excerpt = re.sub(r'<.*?>', '', excerpt.replace('<b<', '').replace('&#269;', 'č').replace('&#263;', 'ć'))
