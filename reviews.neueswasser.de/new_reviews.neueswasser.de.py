...
def process_frontpage(data, context, session):
    cats = data.xpath('//ul[contains(@class, "mainmenu ")]/li')
    for cat in cats:
        ...
        if name not in XCAT:
            ...
            for sub_cat in sub_cats:
                ...
                session.queue(Request(url + '?items=54&sorting=item.feedbackDecimal_desc'), process_prodlist, dict(cat=name + '|' + sub_name))
