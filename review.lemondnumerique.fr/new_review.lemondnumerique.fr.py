...
def run(context, session):
    ...
    session.queue(Request('https://lemondenumerique.ouest-france.fr/tests/', use='curl', force_charset='utf-8', max_age=0), process_revlist, dict())
...
def process_revlist(data, context, session):
    ...
    for rev in revs:
        ...
        session.queue(Request(url, use='curl', force_charset='utf-8', max_age=0), process_review_short, dict(url=url))
