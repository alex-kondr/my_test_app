...
def run(context, session):
    session.queue(Request('https://www.digitaltrends.com/', use='curl', force_charset='utf-8'), process_catlist, dict())


def process_frontpage(data, context, session):
    cats = data.xpath('//ul[contains(@class, "list")]/li[contains(@class, "item")]/a')
    for cat in cats:
