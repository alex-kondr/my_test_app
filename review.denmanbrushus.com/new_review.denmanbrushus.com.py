...
def run(context, session):
    session.queue(Request("https://denmanbrushus.com/", force_charset="utf-8", use='curl'), process_frontpage, dict(cat='Hairbrushes'))
...
def process_frontpage(data, context, session):
    ...
    for cat in cats:
        ...
        if name not in XCAT:
            session.queue(Request(url, force_charset="utf-8", use='curl'), process_prodlist, dict(cat=context['cat'] + '|' + name))
...
def process_product(data, context, session):
    ...
    if pid:
        ...
        session.do(Request(revs_url, force_charset='utf-8', use='curl'), process_reviews, dict(product=product, pid=pid))
...
def process_reviews(data, context, session):
    ...
    revs_cnt = context.get('revs_cnt', revs_json.get('stats', {}).get('count', 0))
    ...
    if revs_cnt and int(revs_cnt) > offset:
        ...
        session.do(Request(next_url, force_charset='utf-8', use='curl'), process_reviews, dict(context, product=product, revs_cnt=revs_cnt, offset=offset, page=next_page))
