...
def run(context: dict[str, str], session: Session):
    session.queue(Request('https://www.guitarplayer.com/reviews', force_charset='utf-8'), process_revlist, dict())
...
def process_revlist(data: Response, context: dict[str, str], session: Session):
    ...
    session.queue(Request(next_url, force_charset='utf-8'), process_revlist, dict(page=page+1))
...
def process_review(data: Response, context: dict[str, str], session: Session):
    ...
    grade_overall = data.xpath('//span[contains(@class, "chunk rating")]/@aria-label').string()
    if grade_overall:
