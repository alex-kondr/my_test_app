...
def run(context, session):
    ...
    session.queue(Request('http://xbox-mag.net/tests/', use='curl', force_charset='utf-8'), process_revlist, dict())


def process_revlist(data, context, session):
    revs = data.xpath('//div[@class="entry-content"]/h2')
    for rev in revs:
        title = rev.xpath('text()').string()
        url = rev.xpath('(following-sibling::*)[1]/li/a').string()
