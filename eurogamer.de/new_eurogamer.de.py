...
def run(context, session):
    ...
    session.queue(Request('https://www.eurogamer.de/reviews', force_charset='utf-8', use='curl'), process_revlist, dict())
...
def process_revlist(data, context, session):
    ...
    for rev in revs:
        ...
        session.queue(Request(url, force_charset='utf-8', use='curl'), process_review, dict(title=title, url=url))
...
def process_review(data, context, session):
    ...
    if next_url:
        ...
        session.do(Request(next_url, force_charset='utf-8', use='curl'), process_review_next, dict(grade_overall=grade_overall, summary=summary, excerpt=excerpt, review=review, product=product, page=2))
...
def process_review_next(data, context, session):
    ...
    next_url = data.xpath('//div[@class="next"]/a/@href').string()
    if next_url:
        session.do(Request(next_url, force_charset='utf-8', use='curl'), process_review_next, dict(context, page=page + 1))
