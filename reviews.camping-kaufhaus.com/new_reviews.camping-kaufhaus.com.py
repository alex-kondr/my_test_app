...
def process_product(data, context, session):
    ...
    for rev in revs:
        ...
        if author:
            author = h.unescape(remove_emoji(author)).replace(u'Ã\\x9F', u'ß').replace(u'Ã\\x9C', u'Ü').replace(u'\xe2\x80\x9c', u'"').replace(u'\xc3\x9c', u'Ü').replace(u'\xc3\x9f', u'ß').strip(' +*,')
            if author:
