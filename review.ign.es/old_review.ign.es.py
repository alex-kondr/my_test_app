...
def process_review(data: Response, context: dict[str, str], session: Session):
    ...
    if not name or name == 'Cinema/TV' or name == 'Amazon' or name == 'TV':
        name = context['title'].strip()
    elif name == 'PC' or name == 'Apple' or name == 'Xbox 360' or name == 'Wii' or name == 'PlayStation 3' or name == 'Retro':
        name = context['title'].strip()
    elif name == 'Samsung':
        name = context['title'].strip()
    ...
    conclusion = data.xpath('//p[preceding-sibling::h3[regexp:test(., "el veredicto", "i")]]/text()').string(multiple=True)
    if not conclusion:
        conclusion = data.xpath('//div[@id="id_text"]//text()[not(contains(., "El veredicto"))][not(ancestor::div[@id="ratingsBox"] or preceding::div[@id="ratingsBox"])][preceding::div[contains(text(), "El veredicto")]]').string(multiple=True)
    if not conclusion:
