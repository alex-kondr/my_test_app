...
def run(context, session):
    session.queue(Request('https://www.createautomation.co.uk', use='curl', max_age=0), process_frontpage, dict())
...
def process_frontpage(data, context, session):
    ...
    for cat in cats:
        ...
        if name not in XCAT:
            session.queue(Request(url, use='curl', max_age=0), process_catlist, dict(cat=name.title()))
...
def process_catlist(data, context, session):
    ...
    for sub_cat in sub_cats:
        ...
        session.queue(Request(url, use='curl', max_age=0), process_prodlist, dict(cat=context['cat'] + '|' + name))
...
def process_prodlist(data, context, session):
    ...
    if next_url:
        session.queue(Request(next_url, use='curl', max_age=0), process_prodlist, dict(context))
...
def process_product(data, context, session):
    ...
    for rev in revs:
        ...
        hlp_yes = rev.xpath('.//span[contains(@id, "spnReview")]//text()[contains(., "of")]').string()
        if hlp_yes:
