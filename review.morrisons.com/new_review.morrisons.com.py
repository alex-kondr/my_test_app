...
def process_category(data, context, session):
    ...
    if data_json:
        ...
        options = """--compressed -X PUT -H 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:147.0) Gecko/20100101 Firefox/147.0' -H 'Accept: application/json; charset=utf-8' -H 'Accept-Language: uk-UA,uk;q=0.9,en-US;q=0.8,en;q=0.7' -H 'Accept-Encoding: deflate' -H 'X-CSRF-TOKEN: 056ca5a2-ceaf-45ed-aae8-0bbc2dd15872' -H 'client-route-id: 2a5bd559-d69b-4c84-bb7b-ec089f5c4e30' -H 'ecom-request-source: web' -H 'ecom-request-source-version: 2.0.0-2026-02-12-09h03m25s-aa743fa3' -H 'page-view-id: 8f308a16-2118-4fa7-a49b-9fac7ca9c64d' -H 'content-type: application/json; charset=utf-8' -H 'Connection: keep-alive' -H 'Cookie: VISITORID=-EtLpt5RqIuDB8UakkI03bBEjowhC16SvTTpR3ETD_5gNtd77oyEgpm5uFIscWNAlnn6xp9lojvBmLJKC3FBLumcIEhPHDOOPs1Q_A==; AWSALB=Sy7Qhv6eTvVUQVqL5zej1RnMIThur2UyPtVkoK49UygZ7xzOAImhKExghtMFckzBv38p5gkiSxqejxM5utpi7z0y7T2y2zBwzgElzBJSDlDieb+c2AZDxupaVk9y; AWSALBCORS=Sy7Qhv6eTvVUQVqL5zej1RnMIThur2UyPtVkoK49UygZ7xzOAImhKExghtMFckzBv38p5gkiSxqejxM5utpi7z0y7T2y2zBwzgElzBJSDlDieb+c2AZDxupaVk9y; OptanonConsent=isGpcEnabled=0&datestamp=Mon+Feb+16+2026+12%3A05%3A58+GMT%2B0200+(%D0%B7%D0%B0+%D1%81%D1%85%D1%96%D0%B4%D0%BD%D0%BE%D1%94%D0%B2%D1%80%D0%BE%D0%BF%D0%B5%D0%B9%D1%81%D1%8C%D0%BA%D0%B8%D0%BC+%D1%81%D1%82%D0%B0%D0%BD%D0%B4%D0%B0%D1%80%D1%82%D0%BD%D0%B8%D0%BC+%D1%87%D0%B0%D1%81%D0%BE%D0%BC)&version=202501.2.0&browserGpcFlag=0&isIABGlobal=false&hosts=&consentId=b62cbd90-562e-4eed-9135-28b58cf73009&interactionCount=1&isAnonUser=1&landingPath=NotLandingPage&groups=C0001%3A1%2CC0003%3A1%2CC0004%3A1%2CC0002%3A1&intType=1&geolocation=UA%3B51&AwaitingReconsent=false; OptanonAlertBoxClosed=2026-02-12T11:31:20.829Z; contentExperienceUserId=6a904135-7147-4fbc-a6f7-5016b18a522e; language=en-GB; AWSALBTG=VsdAdLeLyzJT6SR6bEoi5lU58AJz4X2QewrhA/PNkRSOYoAtKu7uacs5Rnm618bGOGJS2QbKRvQwVCO8ZAFEJwCcMQo4JAIIe5MNOEJQpMuZA6uc1LmAJ+M34X6Zoztgi6yFW7pfVoZzVvbEQBC8hm6SIiEYmMriMb1hvKUnE1P6; AWSALBTGCORS=VsdAdLeLyzJT6SR6bEoi5lU58AJz4X2QewrhA/PNkRSOYoAtKu7uacs5Rnm618bGOGJS2QbKRvQwVCO8ZAFEJwCcMQo4JAIIe5MNOEJQpMuZA6uc1LmAJ+M34X6Zoztgi6yFW7pfVoZzVvbEQBC8hm6SIiEYmMriMb1hvKUnE1P6; global_sid=_bibuOy-2-ryM7lpsGH_cg8ExduDLRl5w4a7v7JYxXEAI2sWC4rf38ouNXkrueINfPEpg9sfjzK8h5_Ff4mcnSdIM8RY7T-mPiYOfg==' -H 'Sec-Fetch-Dest: empty' -H 'Sec-Fetch-Mode: cors' -H 'Sec-Fetch-Site: same-origin' -H 'Priority: u=4' -H 'Pragma: no-cache' -H 'Cache-Control: no-cache' --data-raw '{prods_id}'""".format(prods_id=str(prods_id).replace("u'", '"').replace("'", '"'))
...
def process_reviews(data, context, session):
    ...
    for rev in revs:
        ...
        if excerpt and len(h.unescape(excerpt).replace('\n', '').strip()) > 2:
            if title:
                review.title = h.unescape(remove_emoji(title)).replace('\\x27', "'").replace('\\x26', '').replace('â\\x80¦', '').replace(u'â\x80¦', u'').replace('\xe2\x80\x9c', '"').replace('\xe2\x80\x9d', '"').replace(u'\xe2\x80\x9c', '"').replace(u'\xe2\x80\x9d', '"').replace('\\x', "'").replace(u'â\x80\x98', u"'").replace(u'\xe2\x80\x98', u"'").replace(u'Ã\x80', u'À').replace(u'\xc3\x80', u'À').replace(u'â\x98\x86', u'').replace(u'\xe2\x98\x86', u'').strip()
        else:
            excerpt = title

        if excerpt:
            excerpt = h.unescape(remove_emoji(excerpt)).replace('\n', '').replace('\\x27', "'").replace('\\x26', '').replace('â\\x80¦', '').replace(u'â\x80¦', u'').replace('\xe2\x80\x9c', '"').replace('\xe2\x80\x9d', '"').replace(u'\xe2\x80\x9c', '"').replace(u'\xe2\x80\x9d', '"').replace('\\x', "'").replace(u'â\x80\x98', u"'").replace(u'\xe2\x80\x98', u"'").replace(u'Ã\x80', u'À').replace(u'\xc3\x80', u'À').replace(u'\xe2\x98\x86', u'').strip()
            if len(excerpt) > 2:
