...
def run(context: dict[str, str], session: Session):
    ...
    session.queue(Request('https://hifi.nl/api/solr/content.php?q=&category=Hardware&subCategories=&brands=&years=&from=0&till=10000&order=publishDateDesc&contentType=Recensie&page=1', force_charset="utf-8"), process_revlist, dict(context))
...
def process_product(data: Response, context: dict[str, str], session: Session):
    ...
    if author_name:
        author_url = 'https://hifi.nl/content?auteur=' + author_name.replace(' ', '%20')
