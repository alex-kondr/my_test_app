...
def run(context, session):
    ...
    session.queue(Request('http://www.nintendoworldreport.com/review?region=All&system=All&status=All&type=All', use='curl', force_charset='utf-8'), process_revlist, dict())


def process_revlist(data, context, session):
    revs = data.xpath('//table[@id="results"]/tbody/tr[td]')
    for rev in revs:
        title = rev.xpath('td/a/text()').string()
