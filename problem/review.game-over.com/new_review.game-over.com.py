...
def run(context, session):
    ...
    session.queue(Request('https://www.game-over.com/content/category/reviews/', use='curl', max_age=0), process_revlist, dict(cat='Tech'))
    session.queue(Request('https://www.game-over.com/review/gamereview.php', use='curl', max_age=0), process_catlist, dict())
...
def process_catlist(data, context, session):
    ...
    for cat in cats:
        ...
        session.queue(Request(url, use='curl', max_age=0), process_category, dict(cat='Games|' + name))
...
def process_category(data, context, session):
    ...
    for month in months:
        url = month.string()
        session.queue(Request(url, use='curl', max_age=0), process_revlist, dict(context))
...
def process_review(data, context, session):
    ...
    product = Product()
    product.name = context['title'].replace(' Review', '').replace(' Hands-On Preview', '').replace(u'Ã¢â‚¬â„¢', u"'").replace(u'Ã¢â‚¬â€œ', u'–').replace(u'Ã¢â‚¬Å“', u'“').replace(u'Ã¢â‚¬Â', u'”').replace(u'Ã¢â‚¬', u'—').replace(u'Ã¥', u'å').replace(u'Ã¶', u'ö').replace(u'Ã©', u'é').replace(u'Ã ', u'à').replace(u'Ã¯', u'ï').replace(u'Ã¦', u'æ').replace(u'Ã§', u'ç').replace(u'Â£', u'£').replace(u'Â', u' ').replace(u'\xa0', u' ').strip(u' +*,Â\xa0')
    ...
    review = Review()
    review.type = 'pro'
    review.title = context['title'].replace(u'Ã¢â‚¬â„¢', u"'").replace(u'Ã¢â‚¬â€œ', u'–').replace(u'Ã¢â‚¬Å“', u'“').replace(u'Ã¢â‚¬Â', u'”').replace(u'Ã¢â‚¬', u'—').replace(u'Ã¥', u'å').replace(u'Ã¶', u'ö').replace(u'Ã©', u'é').replace(u'Ã ', u'à').replace(u'Ã¯', u'ï').replace(u'Ã¦', u'æ').replace(u'Ã§', u'ç').replace(u'Â£', u'£').replace(u'Â', u' ').replace(u'\xa0', u' ').strip(u' +*,Â\xa0')
    ...
    if excerpt:
        excerpt = excerpt.replace('�', '').replace(u'Ã¢â‚¬â„¢', u"'").replace(u'Ã¢â‚¬â€œ', u'–').replace(u'Ã¢â‚¬Å“', u'“').replace(u'Ã¢â‚¬Â', u'”').replace(u'Ã¢â‚¬', u'—').replace(u'Ã¥', u'å').replace(u'Ã¶', u'ö').replace(u'Ã©', u'é').replace(u'Ã ', u'à').replace(u'Ã¯', u'ï').replace(u'Ã¦', u'æ').replace(u'Ã§', u'ç').replace(u'Â£', u'£').replace(u'Â', u' ').replace(u'\xa0', u' ').strip(u' +*,Â\xa0')
