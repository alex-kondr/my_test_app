...
def run(context: dict[str, str], session: Session):
    ...
    session.queue(Request('https://tamaris.com/de-CH/', use='curl', force_charset='utf-8'), process_frontpage, dict())
...
def process_prodlist(data: Response, context: dict[str, str], session: Session):
    ...
    for prod in prods:
        ...
        if mpn:
            ...
            session.queue(Request(revs_url, use='curl', force_charset='utf-8'), process_reviews, dict(product=product, mpn=mpn))
...
def process_reviews(data: Response, context: dict[str, str], session: Session):
    ...
    if revs_cnt and offset < int(revs_cnt):
        ...
        session.do(Request(next_url, use='curl', force_charset='utf-8'), process_reviews, dict(context, offset=offset))
