...
def run(context, session):
    ...
    session.queue(Request('https://www.gamereactor.pt/analises/', use='curl', force_charset='utf-8'), process_revlist, {})
...
def process_review(data, context, session):
    ...
    product.manufacturer = data.xpath('//li[span[contains(., "Produtora:")]]//text()[not(parent::span)]').string(multiple=True)
    if not product.manufacturer:
        product.manufacturer = data.xpath('//li[span[contains(., "Editora:")]]//text()[not(parent::span)]').string(multiple=True)
    ...
    genres = data.xpath('//li[span[contains(., "Género:")]]/a/text()').strings()
    if genres:
