...
def run(context, session):
    ...
    session.queue(Request('https://www.musicstore.de/de_DE/EUR'), process_frontpage, dict())
...
def process_subcategory(data, context, session):
    ...
    for subcat in subcats:
        ...
        if name not in XCAT and 'youtube' not in url:
            session.queue(Request(url+'?PageSize=90'), process_subcategory, dict(cat=context['cat']+'|'+name))
...
def process_prodlist(data, context, session):
    ...
    for prod in prods:
        ...
        if revs_cnt:
            ...
            if int(revs_cnt) > 0:
                session.queue(Request(url, use='curl'), process_product, dict(context, name=name, url=url, revs_cnt=int(revs_cnt)))
...
def process_product(data, context, session):
    ...
    if revs_id:
        context['revs_id'] = revs_id.split('PageableID=')[-1].split('&SKU')[0]
