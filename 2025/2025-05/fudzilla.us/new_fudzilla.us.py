...
def run(context, session):
    ...
    session.queue(Request('https://www.fudzilla.com/reviews', use='curl', force_charset='utf-8'), process_revlist, dict())


def process_revlist(data, context, session):
    revs = data.xpath('//h3[@class="catItemTitle"]/a')
    for rev in revs:
    ...
    if revs:
        ...
        session.queue(Request(next_url, use='curl', force_charset='utf-8'), process_revlist, dict(offset=offset))
...
def process_review(data, context, session):
    ...
    if not excerpt:
        excerpt = data.xpath('//div[@class="itemFullText"]/p[not(contains(., "Conclusion"))]//text()[not(regexp:test(., "customers can get |Amazon"))]').string(multiple=True)
    if not excerpt:
