...
def process_revlist(data, context, session):
    revs = data.xpath('//div[p[@data-testid]]')
    for rev in revs:
...
def process_review(data, context, session):
    ...
    grade_overall = data.xpath('//div[div[svg]]//span//text()[not(contains(., "/"))]').string()
    if grade_overall:
    ...
    summary = data.xpath('(//div[div[svg]])[1]/preceding::div[@class and div[div[@class="div-breakable"]]]//p//text()').string(multiple=True)
    if summary:
        review.add_property(type='summary', value=summary)

    conclusion = data.xpath('//div[contains(., "Conclusion")]/following::p[not(preceding::div[contains(text(), "Sous-notes")])]//text()').string(multiple=True)
    if conclusion:
        review.add_property(type='conclusion', value=conclusion)

    excerpt = data.xpath('//div[contains(., "Conclusion")]/preceding::p[@class]//text()').string(multiple=True)
    if not excerpt:
