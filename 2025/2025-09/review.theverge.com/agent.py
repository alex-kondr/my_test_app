...
def process_catlist(data, context, session):
    cats = data.xpath('//p//a[contains(text(), "Reviews")]')
    for cat in cats:
...
def process_revlist(data, context, session):
    revs = data.xpath('//div[contains(@class, "content-cards")]//a')
    for rev in revs:
        title = rev.xpath('text()').string()
        url = rev.xpath('@href').string()

        if title and not re.search(r'I tested \d+|The best ', title):
...
def process_review(data, context, session):
    ...
    product.category = context['cat'].replace('See All ', '').replace(' Reviews', '').strip()
    ...
    if author and author_url:
        author = author.replace('See All by ', '').strip()
