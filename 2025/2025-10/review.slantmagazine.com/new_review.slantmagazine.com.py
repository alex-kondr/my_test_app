...
def run(contex, session):
    ...
    session.queue(Request('https://www.slantmagazine.com/', max_age=0), process_frontpage, dict())
...
def process_revlist(data, context, session):
    ...
    for rev in revs:
        ...
        session.queue(Request(url, max_age=0), process_review, dict(context, url=url))
