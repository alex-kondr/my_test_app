...
def process_catlist(data, context, session):
    ...
    for cat in cats:
        ...
        url = cat.xpath("@data-query").string()
...
def process_review(data, context, session):
    product = Product()
    product.name = context["title"].split("review:")[0].split(" Review")[0].replace("Long Term Test:", "").replace("Long term test:", "").replace("Long-term test:", "").replace(": hands-on preview", "").replace("hands-on preview", "").replace("PureView hands-on", "").replace('Reviewing the ', '').replace('I reviewed the ', '').replace(" preview", "").replace(' review', '').strip()
    ...
    if excerpt:
        excerpt = excerpt.split("Stuff says: ")[0].strip()
