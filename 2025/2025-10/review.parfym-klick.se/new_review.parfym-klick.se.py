...
OPTIONS = """--compressed -H 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:143.0) Gecko/20100101 Firefox/143.0' -H 'Accept-Encoding: deflate' -H 'Connection: keep-alive' -H 'Cookie: cookiePermission=%7B%22Essential%22%3Atrue%2C%22Analytical%22%3Atrue%2C%22Functional%22%3Atrue%2C%22Advertising%22%3Atrue%7D; PHPSESSID=v290adia9e4clfrpbb05cv4uf4; siteType=desktop'"""
...
def run(context, session):
    ...
    session.queue(Request('https://www.parfym-klick.se/', use='curl', force_charset='utf-8', max_age=0, options=OPTIONS), process_frontpage, dict())
...
def process_frontpage(data, context, session):
    ...
    for cat in cats:
        ...
        if name not in XCAT:
            session.queue(Request(url + '?l=144', use='curl', force_charset='utf-8', max_age=0, options=OPTIONS), process_catlist, dict(cat=name))
...
def process_catlist(data, context, session):
    ...
    for sub_cat in sub_cats:
        ...
        session.queue(Request(url, use='curl', force_charset='utf-8', max_age=0, options=OPTIONS), process_prodlist, dict(cat=context['cat'] + '|' + sub_name))
    else:
