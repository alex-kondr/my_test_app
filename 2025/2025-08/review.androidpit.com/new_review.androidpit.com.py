...
def process_review(data, context, session):
    ...
    product.category = data.xpath('//div[@class="articleContentWrapper"]/ul/li/a[not(contains(., "Home") or contains(., "Manufac") or contains(., "More") or contains(., "Master your"))]/text()').string()
    ...
    pros = data.xpath('//ul[@class="goodList"]/li/span[@class="goodBadContent"]')
    for pro in pros:
    ...
    cons = data.xpath('//ul[@class="badList"]/li/span[@class="goodBadContent"]')
    for con in cons:
    ...
    summary = data.xpath('//div[@class="articlePartIntroContent"]/p[not(i[@data-stringify-type="italic"])]//text()').string(multiple=True)
    if summary:
    ...
    excerpt = data.xpath('//div[contains(@class, "articlePartContent")]/p[not(preceding-sibling::h2[regexp:test(., "conclusion|Where to Buy|final thoughts|bottom line", "i")])]//text()').string(multiple=True)
    if excerpt:
