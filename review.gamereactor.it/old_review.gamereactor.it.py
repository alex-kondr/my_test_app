...
def process_product(data: Response, context: dict[str, str], session: Session):
    ...
    name = data.xpath("//h2//a//text()").string()
    if not name:
       name = data.xpath("//h1[@class='noflir']//text()").string()
    product.name = name
    ...
    if not(cat):
        cat = data.xpath("(//ul[@class='infobox']//li[contains(.,'Piattaforma')]//text())[2]").string()
    if cat:
...
def run(context: dict[str, str], session: Session):
    session.queue(Request('https://www.gamereactor.it/gadget', use='curl'), process_productlist, dict())
    session.queue(Request('https://www.gamereactor.it/recensioni', use='curl'), process_productlist, dict())