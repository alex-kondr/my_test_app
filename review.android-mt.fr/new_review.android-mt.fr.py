...
def process_review(data: Response, context: dict[str, str], session: Session):
    ...
    if not product.name:
        product.name = context['title'].replace('Promo et test de', '').replace('Test Express :', '').replace('Test EXCLUSIF du', '').replace('Test de', '').replace('Test du', '').replace('[Test ]', '').replace('Test :', '').replace('Test :', '').replace('Test', '').split(':')[0].split(']')[-1].strip()
    ...
    author = data.xpath('//a[@rel="author"]/span/text()|//li[@itemprop="author"]//span[contains(@class, "author")]/text()[normalize-space(.)]').string()
    author_url = data.xpath('(//a[@rel="author"]|//li[@itemprop="author"]/a)/@href').string()
    ...
    pros = data.xpath('//div[@class="plus"]//li|(//h5[contains(., "Les plus")]/following-sibling::*)[1]/li')
    for pro in pros:
    ...
    cons = data.xpath('(//div[@class="moins"]//li|//h5[contains(., "Les moins")]/following-sibling::*)[1]/li')
    for con in cons:
    ...
    summary = data.xpath('//div[contains(@class, "post-excerpt")]/div[@class="elementor-widget-container"]//text()').string(multiple=True)
    if summary:
    ...
    conclusion = data.xpath('//h2[contains(., "verdict") or contains(., "Verdict") or contains(., "Notre avis")]/following-sibling::p[not(a[@title="Forum Android MT"] or @class or @align="center") or regexp:test(@class, "^p\d+$")]//text()[not(contains(., "[youtube"))]').string(multiple=True)
    if not conclusion:
    ...
    excerpt = data.xpath('//h2[contains(., "verdict") or contains(., "Verdict") or contains(., "Notre avis")]/preceding::p[not(a[@title="Forum Android MT"] or @class or @align="center") or regexp:test(@class, "^p\d+$")]//text()[not(contains(., "[youtube"))]').string(multiple=True)
    if not excerpt:
        excerpt = data.xpath('//h3[contains(., "techniques")]/preceding::p[not(strong[contains(., "verdict")] or a[@title="Forum Android MT"] or @class or @align="center") or regexp:test(@class, "^p\d+$")]//text()|//h3[contains(., "techniques")]/preceding-sibling::text()[not(contains(., "[youtube"))]').string(multiple=True)
    if not excerpt:
