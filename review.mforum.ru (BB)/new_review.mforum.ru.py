...
def run(context: dict[str, str], session: Session):
    ...
    session.queue(Request('https://www.mforum.ru/news/archive.htm?c=news%2Ftests&sn=0', use='curl', force_charset='utf-8'), process_revlist, dict())
...
def process_revlist(data: Response, context: dict[str, str], session: Session):
    ...
    next_url = data.xpath('//a[@title="следующая"]/@href').string()
    if next_url:
