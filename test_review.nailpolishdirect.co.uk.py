...
def process_reviews(data, context, session):
    ...
    for i in range(len(revs)):
        ...
        if i == (len(revs) - 1):
            review.date = data.xpath('//meta[@itemprop="datePublished"][-1]/@content').string()
        else:
            review.date = revs[i+1].xpath('text()').string()
            author = revs[i+1].xpath('.//div[@itemprop="author"]//text()').string()
            review.authors.append(Person(name=author, ssid=author))
