...
def run(context: dict[str, str], session: Session):
    ...
    session.queue(Request('https://www.asadventure.com/nl.html', force_charset='utf-8'), process_frontpage, dict())
...
def process_prodlist(data: Response, context: dict[str, str], session: Session):
    prods = data.xpath('//div[@data-testid="product-tile"]')
    for prod in prods:
        url = prod.xpath('(span/a|a)/@href').string().split('?')[0]

        revs_cnt = prod.xpath('.//div[@data-testid="product-tile-rating"]/div/span/text()').string()
        if revs_cnt and int(revs_cnt) > 0:
            session.queue(Request(url, force_charset='utf-8'), process_product, dict(context, url=url, revs_cnt=int(revs_cnt)))
...
def process_product(data: Response, context: dict[str, str], session: Session):
    ...
    if mpn:
        ...
        session.do(Request(revs_url, use='curl', force_charset='utf-8', max_age=0), process_reviews, dict(context, product=product, mpn=mpn))
