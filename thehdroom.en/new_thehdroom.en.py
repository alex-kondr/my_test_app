...
def run(context: dict[str, str], session: Session):
    ...
    session.queue(Request("https://thehdroom.com/gaming/", use='curl', force_charset='utf-8'), process_revlist, dict())
...
def process_review(data: Response, context: dict[str, str], session: Session):
    product = Product()
    product.name = context['title'].replace(' Hands-On Preview (Xbox 360, PS3)', '').replace('Hands-On Preview: ', '').replace('Hands-On Preview of ', '').replace(': Hands-On Preview', '').split("Review")[0].split('’ ')[0].split(' Trailer ')[0].split(' Preview: ')[0].split(' Preview ')[0].replace(' Preview', '').replace(' hands-on preview', '').replace(' preview', '').strip("''‘’– ")
