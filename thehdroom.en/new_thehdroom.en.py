...
def run(context, session):
    ...
    session.queue(Request("https://thehdroom.com/gaming/", use='curl', max_age=0), process_revlist, dict())
...
def process_revlist(data, context, session):
    ...
    for rev in revs:
        ...
        session.queue(Request(url, use='curl', max_age=0), process_review, dict(context, url=url, title=title))
