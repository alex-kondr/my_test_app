...
def process_reviews(data, context, session):
    revs = data.xpath('//div[div[contains(@class, "paragraph-wrapper") and .//span[regexp:test(., "Platz \d+")]] and following-sibling::div//p]')
    for i, rev in enumerate(revs):
        ...
        product.url = prod_info.xpath('.//a/@href').string()
        ...
        if not grade_overall:
            grade_overall = rev.xpath('count(.//div[contains(@class, "chakra-stack")]/svg[contains(@class, "chakra-icon")]) + count(.//div[contains(@class, "chakra-stack")]/div/div[not(@class="css-1udpgqb")]) div 2')
            if grade_overall:
        ...
        excerpt = rev.xpath('preceding-sibling::div[1][count(preceding-sibling::div[.//span[regexp:test(., "Platz \d+")]])={i} and not(.//img)]//p//text()'.format(i=i)).string(multiple=True)
        if excerpt:
            strings = []
            if 'Fazit:' in excerpt:
