...
def process_review(data, context, session):
    ...
    context['excerpt'] = data.xpath('//div[contains(@class, "entry-content")]//p[not(regexp:test(., "Questions|Comments"))]//text()').string(multiple=True)
...
def process_review_last(data, context, session):
    ...
    if context.get('pages'):
        conclusion = data.xpath('//div[contains(@class, "entry-content")]/p[not(.//strong[regexp:test(., "Pros|Cons")] or preceding-sibling::p[.//strong[regexp:test(., "Pros|Cons")]] or regexp:test(., "Questions|Comments|Conclusion"))]//text()').string(multiple=True)
        if conclusion:
