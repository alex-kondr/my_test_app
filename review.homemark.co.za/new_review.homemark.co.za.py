...
def run(context: dict[str, str], session: Session):
    session.queue(Request("https://homemark.co.za/", force_charset='utf-8', use='curl'), process_frontpage, dict())
...
def process_frontpage(data: Response, context: dict[str, str], session: Session):
    ...
    for cat in cats:
        ...
        for subcat in subcats:
            ...
            session.queue(Request(url, force_charset='utf-8', use='curl'), process_prodlist, dict(cat=name + '|' + subcat_name))
