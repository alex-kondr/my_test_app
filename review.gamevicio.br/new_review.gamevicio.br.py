...
def run(context, session):
    session.queue(Request('https://www.gamevicio.com/analises-de-jogos-games-controles-e-consoles/', force_charset='utf-8'), process_revlist, dict())
...
def process_review(data, context, session):
    product = Product()
    product.name = context['title'].replace('Análise Completa do ', '').replace('Análise – ', '').replace('Análise | ', '').replace('Análise: ', '').replace('OPINIÃO: ', '').replace('OPINIÃO | ', '').replace('#GV Review – ', '').replace('#GV Review: ', '').replace('Review | ', '').strip()
    ...
    if platforms:
        product.category = 'Jogos|' + platforms.replace(' e ', '/').replace(' (possui demo)', '')
