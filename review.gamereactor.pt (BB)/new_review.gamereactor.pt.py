...
def run(context: dict[str, str], session: Session):
    ...
    session.queue(Request('https://www.gamereactor.pt/analises/'), process_revlist, dict())
...
def process_review(data: Response, context: dict[str, str], session: Session):
    ...
    product.manufacturer = data.xpath('//div[contains(@class, "side-sticky")]//div[contains(span, "Produtora")]/span[not(contains(., "Produtora"))]/text()').string(multiple=True)

    title = data.xpath("//h1/text()").string()
    product.name = data.xpath('//div[@class="gr-headgrid"]//div[normalize-space(span/text())="Análises"]/span[not(contains(., "Análises"))]/text()').string()
    if not product.name:
        product.name = title
    ...
    pros = data.xpath('//div[@class="gr-verdict"]//div[normalize-space(span/text())="Prós"]/span[not(normalize-space(text())="Prós")]/text()')
    for pro in pros:
        pro = pro.string()
        if pro:
            pro = pro.strip(' +-*.:;•,–')
    ...
    cons = data.xpath('//div[@class="gr-verdict"]//div[normalize-space(span/text())="Contras"]/span[not(normalize-space(text())="Contras")]/text()')
    for con in cons:
        con = con.string()
        if con:
            con = con.strip(' +-*.:;•,–')
