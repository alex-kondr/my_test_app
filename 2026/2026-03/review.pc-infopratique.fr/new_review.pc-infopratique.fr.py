...
def run(context, session):
    ...
    session.queue(Request('http://www.pc-infopratique.com/dossier-debutr-0.html', use='curl'), process_revlist, dict())
...
def process_review(data, context, session):
    ...
    if pages:
        session.do(Request(page_url, use='curl'), process_review_last, dict(product=product, review=review, excerpt=excerpt))
