...
def run(context, session):
    session.queue(Request("https://techguysmartbuy.com/category/review", use='curl', max_age=0), process_revlist, dict())
...
def process_review(data, context, session):
    ...
    product.url = data.xpath('//a[regexp:test(., "Amazon|Best Buy|Walmart")]/@href').string()
    if not product.url:
        product.url = context['url']

    product.category = data.xpath("//div[@class='single-metainfo']/a[@rel='category tag' and not(regexp:test(., 'review', 'i'))]//text()").string()
    ...
    if not conclusion:
        conclusion = data.xpath('//div[div[contains(@class, "elementor-inner-column") and .//a[contains(., "Amazon")]]]/div//h3[contains(@class, "title")]//text()').string(multiple=True)
    ...
    if excerpt:
        excerpt = h.unescape(excerpt).replace(u'\uFEFF', '').split("Tags: ")[0].split("Comments")[0].strip()
