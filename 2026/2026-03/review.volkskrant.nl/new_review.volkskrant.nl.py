...
def process_revlist(data, context, session):
    ...
    if revs:
        ...
        session.queue(Request('https://www.volkskrant.nl/ajax/recensies/film/leesmeer/' + str(next_page), use='curl', force_charset='utf-8'), process_revlist, dict(page=next_page))
...
def process_review(data, context, session):
    ...
    product = Product()
    product.name = context['title'].strip(' ★☆')
    ...
    if grade_overall:
        grade_overall = float(grade_overall.split('/')[0].split()[0].count('★'))
