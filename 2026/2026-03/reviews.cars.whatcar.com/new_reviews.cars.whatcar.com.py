...
def process_revlist(data, context, session):
    ...
    revs = data.xpath('//div[contains(@class, "cars-card") and div/h3]')
    for rev in revs:
        name = rev.xpath('div/h3[contains(@class, "card-model")]/text()').string()
        url = rev.xpath('a[contains(@class, "card-link")]/@href').string()
