...
def run(context, session):
    ...
    session.queue(Request('https://www.all-nintendo.com/category/tests/', force_charset='utf-8'), process_revlist, dict())
...
def process_review(data, context, session):
    ...
    if not conclusion:
        conclusion = data.xpath('//p[contains(strong, "Verdict")]/following-sibling::p[not(regexp:test(., "Qu’en pensez-vous ?|Points positifs|Points négatifs|Les plus :|Les plus:|Les moins :|Les moins:|Je le recommande aux :|Je le recommande aux:|Évitez si vous cherchez :|Évitez si vous cherchez:"))]//text()[not(regexp:test(normalize-space(.), "\d+/\d+|^\+$|^–|^-$|Note:|Note :") or regexp:test(normalize-space(.), "^–|^-"))]').string(multiple=True)
    ...
    if not excerpt:
        excerpt = data.xpath('//p[contains(strong, "Verdict")]/preceding-sibling::p[not(regexp:test(., "Qu’en pensez-vous ?|Points positifs|Points négatifs|Les plus :|Les plus:|Les moins :|Les moins:|Je le recommande aux :|Je le recommande aux:|Évitez si vous cherchez :|Évitez si vous cherchez:"))]//text()[not(regexp:test(normalize-space(.), "\d+/\d+|^\+$|^–|^-$|Note:|Note :") or regexp:test(normalize-space(.), "^–|^-"))]').string(multiple=True)
    if not excerpt:
