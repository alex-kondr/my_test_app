...
def run(context, session):
    ...
    session.queue(Request('https://www.frogu.cz/', use='curl', force_charset='utf-8'), process_frontpage, {})
...
def process_revlist(data, context, session):
    ...
    for rev in revs:
        ...
        session.queue(Request(url, use='curl', force_charset='utf-8'), process_review, dict(context, title=title, url=url, ssid=ssid))
