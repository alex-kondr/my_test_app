...
def run(context: dict[str, str], session: Session):
    session.queue(Request('https://homekitnews.com/category/reviews/', force_charset='utf-8', use='curl'), process_revlist, dict())


def process_revlist(data: Response, context: dict[str, str], session: Session):
    revs = data.xpath('//div[contains(@class, "post-content")]')
    for rev in revs:
        title = rev.xpath('.//h2[@class="cm-entry-title"]/a/text()').string()
        author = rev.xpath('//span[contains(@class, "author")]/a[not(contains(@href, "simon"))]').first()
        url = rev.xpath('.//h2[@class="cm-entry-title"]/a/@href').string()
        session.queue(Request(url, force_charset='utf-8', use='curl'), process_review, dict(title=title, url=url, author=author))
...
def process_review(data: Response, context: dict[str, str], session: Session):
    ...
    if context['author']:
        author = context['author'].xpath('text()').string()
        author_url = context['author'].xpath('@href').string()
    else:
