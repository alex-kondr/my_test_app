...
def process_frontpage(data,context, session):
    ...
    for cat in cats:
        ...
        if name:
            cats1 = cat.xpath('.//ul[contains(@class, "child-nav")]/li')

            if cats1:
                for cat1 in cats1:
                    cat1_name = cat1.xpath('div/a/text()').string(multiple=True)

                    if cat1_name:
                        subcats = cat1.xpath('.//ul[contains(@class, "nav__grandchild")]/li/a')
                        if subcats:
...
def process_prodlist(data: Response, context: dict[str, str], session: Session):
    ...
    for prod in prods:
        ...
        revs_cnt = prod.xpath('.//span[contains(@class, "prev-badge__text")]/text()').string()
        if revs_cnt:
            revs_cnt = revs_cnt.split()[0].strip()
            if revs_cnt.isdigit() and int(revs_cnt) > 0:
                session.queue(Request(url, force_charset='utf-8', max_age=0), process_product, dict(context, name=name, url=url, revs_cnt=int(revs_cnt)))
