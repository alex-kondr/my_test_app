...
XCAT = ['Unsere Topseller', 'Handy mit Vertrag', 'Handytarife', 'Kundenservice', 'Apple Refurbished', 'Tablet mit Vertrag', 'Datentarife', 'Top Angebote', 'Sommer-Specials']
...
def run(context: dict[str, str], session: Session):
    ...
    session.queue(Request('https://www.gomibo.de/', use='curl', force_charset='utf-8'), process_frontpage, dict())
...
def process_prodlist(data: Response, context: dict[str, str], session: Session):
    ...
    for prod in prods:
        ...
        revs_cnt = prod.xpath('.//span[regexp:test(., "^\(\d+\)")]/text()').string()
        if revs_cnt:
