...
URLS = ['https://www.latercera.com/pf/api/v3/content/fetch/story-feed-sections-fetch?query={"feedOffset":offset,"feedSize":5,"fromComponent":"grid","includeSections":"/mouse","query":"/mouse","tagSlug":"/mouse"}&d=1223&mxId=00000000&_website=la-tercera', 'https://www.latercera.com/pf/api/v3/content/fetch/story-feed-sections-fetch?query={"feedOffset":offset,"feedSize":5,"fromComponent":"grid","includeSections":"/tecnologia","query":"/tecnologia","tagSlug":"/tecnologia"}&d=1223&mxId=00000000&_website=la-tercera']


def run(context: dict[str, str], session: Session):
    session.sessionbreakers = [SessionBreak(max_requests=4000)]
    for url in URLS:
        session.queue(Request(url.replace('offset', '0'), use='curl', force_charset='utf-8'), process_revlist, dict(cat_url=url))
...
def process_revlist(data: Response, context: dict[str, str], session: Session):
    ...
    if offset < revs_cnt:
        session.queue(Request(context['cat_url'].replace('offset', str(offset)), use='curl', force_charset='utf-8'), process_revlist, dict(context, revs_cnt=revs_cnt, offset=offset))
