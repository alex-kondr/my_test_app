...
def run(context: dict[str, str], session: Session):
    ...
    for link in cats_links:
        ...
        session.queue(Request(url, max_age=0, use='curl', force_charset='utf-8'), process_prodlist, dict(link=link))
...
def process_prodlist(data: Response, context: dict[str, str], session: Session):
    ...
    for prod in prods:
        ...
        if revs_cnt and int(revs_cnt) > 0:
            ...
            session.queue(Request(revs_url, max_age=0, use='curl', force_charset='utf-8'), process_reviews, dict(product=product, revs_cnt=int(revs_cnt)))
    ...
    if offset < prods_count:
        ...
        session.queue(Request(next_url, max_age=0, use='curl', force_charset='utf-8'), process_prodlist, dict(context, offset=offset, prods_count=prods_count))
...
def process_reviews(data: Response, context: dict[str, str], session: Session):
    ...
    if offset < context['revs_cnt']:
        ...
        session.do(Request(next_url, max_age=0, use='curl', force_charset='utf-8'), process_reviews, dict(context, product=product, offset=offset, page=next_page))
