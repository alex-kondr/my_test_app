...
def run(context, session):
    ...
    session.queue(Request('https://playfront.de/category/reviews/'), process_revlist, dict())


def process_revlist(data, context, session):
    revs = data.xpath('//div[@data-pid]')
    for rev in revs:
...
def process_review(data, context, session):
    product = Product()
    product.name = context['title'].split('Test: ')[-1].split('TEST: ')[-1].split('TEST – ')[-1].split('Kurztest – ')[-1].split(' – ')[0].replace('Review: ', '').replace('Kurztest: ', '').replace('Leser-Review: ', '').replace('PlayLink Kompakttest: ', '').replace('im Review ', '').replace('TEST:', '').replace(u'\u00a0', '').replace('REVIEW: ', '').replace(' im Test: ', '').replace(' im Test', '').strip(' -')
    ...
    if not conclusion:
        conclusion = data.xpath('//blockquote[preceding-sibling::*[self::h2[@id="fazit"] or self::p[.="Fazit" or .="FAZIT"]]]//text()').string(multiple=True)
    if not conclusion:
