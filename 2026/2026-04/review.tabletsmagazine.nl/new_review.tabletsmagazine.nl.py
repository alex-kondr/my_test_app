...
def run(context, session):
   session.queue(Request('http://www.tabletsmagazine.nl/category/tablet-reviews/', max_age=0), process_revlist, dict())
...
def process_revlist(data, context, session):
    ...
    for rev in revs:
        ...
        session.queue(Request(url, max_age=0), process_review, dict(cats=cats, title=title, url=url))
