...
def run(context, session):
    ...
    session.queue(Request('https://www.deltachildren.com/', use='curl'), process_frontpage, dict())
    session.queue(Request('https://www.deltachildren.com/collections/wagons', use='curl'), process_prodlist, dict(cat='Wagons'))
...
def process_product(data, context, session):
    ...
    if revs_cnt:
        ...
        if revs_cnt.isdigit() and int(revs_cnt) > 0:
            ...
            session.do(Request(revs_url, use='curl', max_age=0), process_reviews, dict(product=product))
