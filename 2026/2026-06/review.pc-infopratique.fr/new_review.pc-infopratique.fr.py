...
def run(context, session):
    ...
    session.queue(Request('http://www.pc-infopratique.com/dossier-debutr-0.html', max_age=0), process_revlist, dict())
...
def process_review(data, context, session):
    ...
    if date:
        review.date = date.split(' ', 1)[-1].replace(u'�', u'e')
    ...
    if conclusion:
        conclusion = conclusion.replace('Conclusion: ', '').replace('Conclusion ', '').strip(' :')
    ...
    if pages:
        session.do(Request(page_url, max_age=0), process_review_last, dict(product=product, review=review, excerpt=excerpt))
