...
def run(context, session):
    ...
    session.queue(Request('https://www.camping-kaufhaus.com/widgets/menu/offcanvas?navigationId=0199ecdbb9af73138656afcb301073e7', force_charset='utf-8', use='curl'), process_catlist, dict())
...
def process_category(data, context, session):
    ...
    subcats = data.xpath('//ul[contains(@class, "level-1")]/li/a[contains(@class, "category-navigation-link")]')
    for subcat in subcats:
        name = subcat.xpath('text()').string()
        url = subcat.xpath('@href').string()
        session.queue(Request(url, force_charset='utf-8', use='curl'), process_prodlist, dict(cat=context['cat']+'|'+name, cat_url=url))
...
def process_product(data, context, session):
    ...
    product.manufacturer = data.xpath('//img[contains(@class, "manuLogoDesktop")]/@alt').string()
