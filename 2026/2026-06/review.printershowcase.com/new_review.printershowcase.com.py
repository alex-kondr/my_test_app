...
def run(context, session):
    ...
    session.queue(Request('https://www.printershowcase.com/color-laser-printer-reviews.aspx', use='curl', max_age=0), process_catlist, dict(cat='Color Laser Printer|'))
...
def process_revlist(data, context, session):
    ...
    for rev in revs:
        ...
        session.queue(Request(url, use='curl', max_age=0), process_review, dict(cat=context['cat']+subcat_name, url=url))
