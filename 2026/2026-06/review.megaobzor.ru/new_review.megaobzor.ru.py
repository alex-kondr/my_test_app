...
def run(context, session):
    ...
    session.queue(Request('http://megaobzor.com/news-topic-15-page-1.html', use='curl', force_charset='utf-8'), process_revlist, dict())
...
def process_review(data, context, session):
    ...
    if not review.authors:
        author = data.xpath('//text()[contains(., "Автор - ")]/following-sibling::span[1]/text()').string(multiple=True)
        if author:
            review.authors.append(Person(name=author, ssid=author))
