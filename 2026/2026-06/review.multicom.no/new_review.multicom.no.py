...
def run(context, session):
    ...
    session.queue(Request('https://www.multicom.no/', force_charset='utf-8'), process_frontpage, {})
                                ...
                                if cats4:
                                    for cat4 in cats4:
                                        ...
                                        if name4 and name4 not in XCAT:
                                            ...
                                            session.queue(Request(url, force_charset='utf-8'), process_category, dict(cat=name1+'|'+name2+'|'+name3+'|'+name4))

                                elif name1 and name2 and name3:
                                    url = cat3.xpath('a/@href').string()
                                    session.queue(Request(url, force_charset='utf-8'), process_category, dict(cat=name1+'|'+name2+'|'+name3))

                    elif name1 and name2:
                        url = cat2.xpath('a/@href').string()
                        session.queue(Request(url, force_charset='utf-8'), process_category, dict(cat=name1+'|'+name2))
...
def process_category(data, context, session):
    ...
    for prod in prods:
        ...
        session.queue(Request(url, force_charset='utf-8'), process_product, dict(context, name=name, manufacturer=manufacturer, url=url))

    next_url = data.xpath('//link[@rel="next"]/@href').string()
    if next_url:
        session.queue(Request(next_url, force_charset='utf-8', max_age=0), process_category, dict(context))
...
def process_product(data, context, session):
    ...
    if revs_cnt:
        ...
        if revs_cnt > 0:
            ...
            session.do(Request(revs_url, use='curl', force_charset='utf-8', max_age=0), process_reviews, dict(product=product, revs_cnt=revs_cnt, mpn=mpn))
