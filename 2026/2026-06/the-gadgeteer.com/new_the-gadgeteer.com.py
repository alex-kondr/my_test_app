...
def run(context, session):
    ...
    session.queue(Request('https://the-gadgeteer.com/category/reviews/'), process_revlist, dict())
...
def process_revlist(data, context, session):
    ...
    next_url = data.xpath('(//link[@rel="next"]|//a[contains(@class, "next")])/@href').string()
    if next_url:
