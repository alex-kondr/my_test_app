...
def RequestRevs(offset, token):
    url = 'https://www.pickr.com.au/wp-admin/admin-ajax.php'
    payload = {
        'action': 'pickrc_load_more_posts',
        'nonce': token,
        'offset': str(offset),
        'per_page': '5',
        'category_id': '157',
    ...
    time_1 = int(time.time()) + 3600
    time_2 = int(time.time()) + 4300
    r = Request(url, data=payload, method='POST')
    r.add_header('Accept-Encoding', 'deflate')
    r.add_header('Cookie', '_color_system_schema=dark; pickrc-retina=true; icwp-wpsf-notbot=notbotZaltchaZexp-{time_1}; icwp-wpsf-notbot=notbotZaltchaZexp-{time_2}'.format(time_1=time_1, time_2=time_2))
    return r
...
def run(context, session):
    ...
    session.queue(Request('https://www.pickr.com.au/reviews/'), process_category, dict())


def process_category(data, context, session):
    strip_namespace(data)

    token = data.xpath('//script[contains(., "var pickrcData = ")]/text()').string()
    if token:
        token = token.split('"nonce":"')[-1].split('",')[0]
        session.do(RequestRevs(0, token), process_revlist, dict(token=token))
...
def process_revlist(data, context, session):
    ...
    revs = new_data.xpath('//article[contains(@id, "post-")]')
    for rev in revs:
    ...
    if revs_json.get('hasMore') is True:
        session.do(RequestRevs(offset, context['token']), process_revlist, dict(context, offset=offset))
...
def process_review(data, context, session):
    ...
    if not grade_overall:
        grade_overall = data.xpath('//span[@class="score__number"]/text()').string()
    ...
    summary = data.xpath('//div[@class="post-hero-text"]/p//text()').string(multiple=True)
    if summary:
    ...
    if not excerpt:
        excerpt = data.xpath('//main[contains(@class, "content")]/p//text()').string(multiple=True)
