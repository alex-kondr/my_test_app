...
def run(context, session):
    session.queue(Request('http://www.canadianreviewer.com/cr/category/reviews'), process_revlist, dict())
...
def process_revlist(data, context, session):
    ...
    revs_cnt = context.get('revs_cnt', data.xpath('//h2[not(@class) and contains(text(), "Reviews")]/text()').string())
    if revs_cnt:
        ...
        if offset < int(revs_cnt):
            ...
            session.queue(Request(next_url), process_revlist, dict(revs_cnt=revs_cnt, offset=offset, page=next_page))
...
def process_review(data, context, session):
    ...
    conclusion = data.xpath('//p[strong[contains(., "Conclusion")]]/following-sibling::p[not(strong[regexp:test(., "Pros|Cons|Rating|Conclusion|Hits|Misses|The Verdict")] or contains(., "Rating:"))]//text()').string(multiple=True)
    if not conclusion:
