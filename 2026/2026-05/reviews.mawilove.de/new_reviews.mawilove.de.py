...
def run(context, session):
    session.queue(Request('https://www.mawilove.de/collections/produkte', force_charset='utf-8'), process_prodlist, dict())


def process_prodlist(data, context, session):
    prods = data.xpath('//div[@class="card__content" and .//div[@data-number-of-reviews]]')
    for prod in prods:
        name = prod.xpath('.//div[@class="card__information"]/p//text()').string(multiple=True)
