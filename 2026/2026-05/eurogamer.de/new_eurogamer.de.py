...
def run(context, session):
    ...
    session.queue(Request('https://www.eurogamer.de/reviews', force_charset='utf-8'), process_revlist, dict())
...
def process_review(data, context, session):
    ...
    if next_url:
        ...
        session.do(Request(next_url, force_charset='utf-8'), process_review_next, dict(grade_overall=grade_overall, summary=summary, excerpt=excerpt, review=review, product=product, page=2))
...
def process_review_next(data, context, session):
    ...
    next_url = data.xpath('//div[@class="next"]/a/@href').string()
    if next_url:
        session.do(Request(next_url, force_charset='utf-8'), process_review_next, dict(context, page=page + 1))
