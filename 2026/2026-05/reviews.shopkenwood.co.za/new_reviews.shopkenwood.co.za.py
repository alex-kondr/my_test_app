...
def run(context, session):
    ...
    session.queue(Request('https://shopkenwood.co.za/', use='curl'), process_frontpage, dict())
...
def process_reviews(data, context, session):
    ...
    page_cnt = context.get('page_cnt', revs_json.get('pagination', {}).get('total_pages'))
    ...
    if next_page <= page_cnt:
        ...
        session.do(Request(revs_url, max_age=0), process_reviews, dict(product=product, page=next_page, page_cnt=page_cnt))
