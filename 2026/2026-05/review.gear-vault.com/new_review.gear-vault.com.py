...
def run(context, session):
    session.queue(Request("http://gear-vault.com/guitars/"), process_revlist, dict(cat='Guitars'))
    session.queue(Request("http://gear-vault.com/amplifiers/"), process_revlist, dict(cat='Amplifiers'))
...
def process_revlist(data, context, session):
    ...
    for rev in revs:
        ...
        if 'review' in title.lower() and not re.search(r'\d+ .+ Guitars In Review', title, re.I):
...
def process_review(data, context, session):
    product = Product()
    product.name = context['title'].replace('Reviewing the ', '').replace(' [Review]', '').split(' Review – ')[0].replace(" Review", '').replace(' review', '').strip()
    ...
    if not excerpt:
        excerpt = data.xpath('//div[contains(@class, "entry-content")]/p[not(contains(., "Written by "))]//text()|//div[contains(@class, "entry-content")]/text()|//div[contains(@class, "entry-content")]/strong//text()|//div[contains(@class, "entry-content")]/a//text()').string(multiple=True)
