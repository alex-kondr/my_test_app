...
def run(context, session):
    ...
    session.queue(Request('https://jeux-video.krinein.com/', use='curl', force_charset='utf-8'), process_revlist, dict())
...
def process_review(data, context, session):
    ...
    genres = data.xpath('(//div[contains(text(), "Genre")]/following-sibling::ul)[1]/li//text()[normalize-space(.)]').strings()
    if genres:
