...
def run(context, session):
    ...
    session.queue(Request('https://www.fotografuj.pl/Dzial/Testy_praktyczne', force_charset='utf-8'), process_revlist, dict())
...
def process_review(data, context, session):
    ...
    product = Product()
    product.name = context['title'].split(' – ')[0].replace('TEST: ', '').replace(u'\uFEFF', '').replace(': pierwsze wrażenia i zdjęcia testowe', '').replace('Test amatorskich lustrzanek Nikona,', '').replace(' - pierwsze zdjęcia testowe!', '').replace(' - pierwsze wrażenia i zdjęcia testowe', '').replace('Pierwsze wrażenia i zdjęcia testowe: ', '').replace('Pierwsze wrażenia, pierwsze testy: ', '').replace('Pierwsze zdjęcia testowe z ', '').replace('Pierwsze wrażenia, pierwsze testy - ', '').replace('Test: ', '').strip()
    ...
    excerpt = data.xpath('(//div[@class="article_body_ramka_in"]|//div[@class="article_body_ramka_in"]/span[@class="bold"])/text()[not(preceding::span[@class="bold" and regexp:test(., "Cena:|Strona producenta")] or regexp:test(., "Cena:|Strona producenta|Plusy|Minusy|Zobacz także:") or contains(., "[a href="))]').string(multiple=True)
    ...
    if pages:
        session.do(Request(page_url, force_charset='utf-8'), process_review_lastpage, dict(excerpt=excerpt, review=review, product=product))
...
def process_review_lastpage(data, context, session):
