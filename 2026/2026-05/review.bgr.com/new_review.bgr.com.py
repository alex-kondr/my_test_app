...
def run(context, session):
    session.queue(Request('https://www.bgr.com/category/reviews/', use='curl'), process_catlist, dict())


def process_catlist(data, context, session):
    cats = data.xpath('//div[@class="subcategories"]/ul//a')
    for cat in cats:
...
def process_review(data, context, session):
    ...
    grade_overall = data.xpath('//div[@class="score"]/span/text()').string()
    if not grade_overall:
        grade_overall = data.xpath('//span[@class="rating"]/text()').string()
