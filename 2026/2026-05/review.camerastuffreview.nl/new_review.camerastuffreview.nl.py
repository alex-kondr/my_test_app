...
def process_review(data, context, session):
    ...
    product = Product()
    product.name = title.replace('Full review: ', '').replace('Full Review: ', '').replace('Preview: ', '').replace('(P)review: ', '').replace('(P)review ', '').replace('Review: ', '').replace('REVIEW: ', '').replace('Full Review ', '').replace('Full review ', '').replace('Review ', '').replace(u'Review\u00a0', '').replace(' review', '').replace('Test: ', '').replace('TEST: ', '').replace('Test ', '').replace('TEST ', '').replace('test ', '').split(' – ')[0].split(' - ')[0].split(':')[0].split('｜')[0].strip(' ￼')
    ...
    if not grade_overall:
        grade_overall = data.xpath('//tr[contains(td, "eindwaardering") or contains(td, "Eindwaardering")]/td[regexp:test(., "\d+")]//text()').string(multiple=True)

    if grade_overall:
        grade_overall = grade_overall.split()[0].replace(',', '.').strip(' +-–')
        try:
    ...
    for grade in grades:
        ...
        if grade_name:
            grade_name, grade_best = grade_name.rsplit('/', 1)
        if grade_val and '*' in grade_val:
            grade_val = grade_val.count('*')
        elif grade_val:
            grade_val = grade_val.replace(',', '.').strip(' +-–')

        if grade_name and grade_val and grade_best:
            grade_name = grade_name.strip(' 510')
            try:
    ...
    conclusions = data.xpath('//div[@data-id="12946e9" or @data-id="97121bd"]//p//text()[normalize-space()][not(contains(., "Conclusie"))]').strings()
    if not conclusions:
    ...
    if not conclusions:
        conclusions = data.xpath('(//h2|//h3)[contains(., "Conclusie")]/following-sibling::p[not(contains(., "insertgrid") or .//span[@class] or @class or contains(., "LAATSTE BERICHTEN FOCUS MAGAZINE"))]//text()[normalize-space()]').strings()
    if not conclusions:
        conclusions = data.xpath('(//h2|//h3)[contains(., "Conclusie")]/following-sibling::div/p[not(contains(., "insertgrid") or .//span[@class] or @class or contains(., "LAATSTE BERICHTEN FOCUS MAGAZINE"))]//text()[normalize-space()]').strings()
    ...
    if not excerpt:
        excerpt = data.xpath('//div[@class="elementor-widget-container"]//p[not(@class or preceding::*[self::h2 or self::h3 or self::p/strong][contains(., "Conclusie")])]//text()').string(multiple=True)
