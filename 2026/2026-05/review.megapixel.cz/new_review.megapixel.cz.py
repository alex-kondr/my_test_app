...
def run(context, session):
    ...
    session.queue(Request('https://www.megapixel.cz/eshop', max_age=0, use='curl'), process_catlist, dict())
...
def process_product(data, context, session):
    ...
    session.do(Request(product.url+'/recenze', max_age=0, use='curl'), process_reviews, dict(product=product))
