...
def run(context, session):
    session.queue(Request('https://www.independentgolfreviews.com/', use='curl', force_charset='utf-8', max_age=0), process_frontpage, dict())
...
def process_frontpage(data, context, session):
    ...
    for cat in cats:
        ...
        for sub_cat in sub_cats:
            ...
            session.queue(Request(url, use='curl', force_charset='utf-8', max_age=0), process_revlist, dict(cat=name+'|'+sub_name, cat_url=url))
...
def process_review(data, context, session):
    ...
    pros = data.xpath('//p[.//strong[regexp:test(., "Quick Hits|Pros")]]/text()')
    for pro in pros:
